...
> **Обновлён:** 2026-05-05 23:50 (UTC+3)
...
| **STEP** | `1.8-B — Runtime Consistency & Security Hardening` |
| **AGENT STATUS** | `READY_FOR_OWNER_ACCEPTANCE` |
| **LAST COMPLETED** | `Phase 1.8-B implementation complete. Fixes C-1 (resolveUserId column), C-2 (SECDEF search_path), M-1 (stale TRANSACTION_TYPE).` |
| **BLOCKER** | None |
| **NEXT ACTION** | Await owner acceptance of Phase 1.8-B. Do NOT start next phase until ACCEPTED. |
...
| 1.8-B Runtime Consistency & Security Hardening | 🔄 READY FOR ACCEPTANCE | C-1: `draft.service.ts` `telegram_user_id`→`telegram_id` fix. C-2: `migrations/1778008400000_harden-onboarding-search-path.js` — `search_path` fixed for 2 SECDEF functions. M-1: `shared/index.ts` `TRANSACTION_TYPE` updated to 5 canonical values. `smoke-test-phase18b.mjs` — 16/16 PASS, 171/171 total regression PASS |
...
> Phase 1.8-B (Runtime Consistency & Security Hardening) is READY_FOR_OWNER_ACCEPTANCE.
