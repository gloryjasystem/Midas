...
| Параметр | Значение |
|---|---|
| **PHASE** | `2.8 — Seamless Account Creation from Draft Picker` |
| **STEP** | `All 4 stages implemented + TS build fix deployed ✅` |
| **AGENT STATUS** | `DONE. Fixed ia:newac callback, added ia:showpicker back-nav, removed success banners, fixed TS6133 unused-var errors. Commit 56991be pushed to main.` |
| **DEPLOYMENT** | `Railway (spirited-happiness project)` — `Midas` ✅ Online · `background-workers` ✅ Online · `Postgres` ✅ · `Redis` ✅. Phase 2.8 deployed. Health: https://midas-production-f4f1.up.railway.app/health → {"status":"ok"} |
| **LAST COMPLETED** | `Phase 2.8 COMPLETE — ia:newac fix + ACCOUNTS_EMPTY_TEXT + ia:showpicker + success banner removal + TS fix. Commits dd0a5e9 → 56991be pushed to main.` |
| **BLOCKER** | None. |
...
| **Phase 2.6 — AI Parser Hardening & V2 UI Sync** | ✅ COMPLETE | **AI Parser (3-Layer Defense):** Prompts (`CURRENCY NORMALIZATION`), Schemas (`currency.catch(undefined)`), Webhook (fall-through to parser during onboarding). **UI Sync:** Re-compiled `dist/` explicitly for all apps so Railway stops caching old `dist`. **Account Picker V2:** Button `Без счёта` replaced with `➕ Создать счёт` (`ia:newac`) and `✖️ Отмена` (`ia:pk:back`). Commits `f53ec09` → `aef9b06` → `e23b356`. |
| **Phase 2.7 — UX Polish & Bugfixes** | ✅ DEPLOYED | **Account Picker Fix:** Исправлен критический баг коммита `6efe173`, когда при отсутствии счетов транзакции молча падали (теперь всегда проверяется наличие счетов перед показом пикера). **Draft Cancellation:** Вместо запутанной кнопки «Записать без счёта» на экране "счёт не найден" внедрена кнопка `✖️ Отмена` (`ia:cancel`). При нажатии: in-place edit сообщения на "❌ Отменено", атомарный SQL `UPDATE status = 'rejected'`, очистка Redis-ключей. **Ghost Message Fix:** Добавлен auto-cleanup в `upsertBotMessage` (если in-place edit карточки отмены невозможен из-за удалённой клавиатуры, она удаляется и пишется новая карточка снизу). **AI Budget:** Увеличен лимит токенов с 500k до 2M в Railway для разблокировки очереди `ai-parse`. Commits `6efe173` → `9c2b80e`. |
| **Phase 2.8 — Seamless Account Creation from Draft Picker** | ✅ DEPLOYED | **Этап 1 — Callback Fix:** `account-inline-keyboard.service.ts` — исправлен критический баг: кнопка «➕ Создать счёт» вызывала `ia:rename` вместо `ia:newac`. Добавлены `showpicker` в `InlineAccountCmd` union и парсер. **Этап 2 — Текст онбординга:** `webhook.route.ts` — стартовый экран выбора типа счёта теперь использует `ACCOUNTS_EMPTY_TEXT` (вместо жёстко прописанного текста) — единый стиль с `/start`. **Этап 3 — Back Navigation:** `account-onboard-keyboard.service.ts` — кнопка «◀️ Назад» на экране выбора типа счёта теперь вызывает `ia:showpicker` (вместо `ia:pk:back`). `webhook.route.ts` — новый handler `ia:showpicker`: восстанавливает `midas:prev_acct` из Redis, рендерит Account Picker без потери контекста черновика. **Этап 4 — Убраны success-баннеры:** Удалены строки `«✅ Счёт создан! ...»` во всех трёх путях создания счёта (`bal_skip`, `bal_input`, `cur_input`) — переход сразу к preview-карточке черновика. **Fix — TS6133:** Убраны неиспользуемые переменные `linkedAccountNameBal` и `acNameBi2`, ставшие «висячими» после удаления success-баннеров. Commits `dd0a5e9` → `56991be`. tsc 0 ошибок · Railway deployed. |
