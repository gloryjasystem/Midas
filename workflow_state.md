...
| �������� | �������� |
|---|---|
| **PHASE_OLD_TX_HUB** | `[archived] Transaction Hub UX � Variant D Icon Chips ? DEPLOYED` |
| **PHASE** | `Phase 2.6 — State-Aware Reminders + Chat Hygiene + item_name UX → DEPLOYED` |
| **STEP** | `Commits cc26652 → 7ea5bf2 pushed to main. Railway auto-deploy ✅.` |
| **AGENT STATUS** | `tsc 0 errors. current_screen column added. Reminders mirror live UI state (screen1/screen1b/screen2). accountCurrency propagated for button labels. item_name rendered as separate paragraph. Stale card cleanup via deleteMessageId. screen-builder.ts formatted and pushed.` |
...
| **LAST COMPLETED** | `Phase 2.6: state-aware reminders, chat hygiene (deleteMessageId), currency label fix (accountCurrency), item_name separate paragraph. Commits cc26652 + 7ea5bf2.` |
| **BLOCKER** | None. |
| **NEXT ACTION** | Monitor draft-expiration worker logs in production — verify reminders correctly mirror screen1b/screen2 states. Verify stale card cleanup works in private chats. |
| **AGENT_STATUS_OLD** | `[archived] tsc 0 errors. IntentFilter 5 ����� (e/i/d/t/a). 'd' = merged ����� (debt_given + debt_received). ������� D: 1 ������ ? 5 ��������� ������ [??][??][??][??][?? ���]. Toggle: ������� �� �������� ������ ������� ���.` |
| **DEPLOYMENT_OLD** | `[archived] Railway (spirited-happiness project)` � `Midas` ? Online � `background-workers` ? Online � `Postgres` ? � `Redis` ?. Health: https://midas-production-f4f1.up.railway.app/health > {"status":"ok"} |
| **LAST_COMPLETED_OLD** | `[archived] Transaction Hub Variant D: ��������� ���� ���������� [??][??][??][??][?? ���] � 1 ������ ������ 2?3 �����. IntentFilter d = SQL IN ('debt_given','debt_received'). Toggle UX. Backward compat dg/dr>d. Commit d770ca4.` |
| **NEXT_ACTION_OLD** | `[archived] Smoke-test Transaction Hub � ����� ����: ��������� ��� 5 ����� ���������, ���������� ���������, toggle ��������. |
...
| 0.3.1 Security & Traceability Patch | ? | SEC-01 � SEC-12 ������� � scope, DB model, queue model, acceptance criteria, ADR-009, ADR-013 |
| 1.1 Project Infrastructure Foundation | ? | `midas-monorepo/` � ������ ��������� Turborepo, Docker Compose, ESLint, TypeScript |
...
| 1.28 /edit Transactions MVP | ? ACCEPTED | `edit.service.ts` (NEW), `edit-keyboard.service.ts` (NEW), `webhook.route.ts` (MODIFY), `confirmation.worker.ts` (MODIFY), `smoke-test-phase128.mjs` (NEW). /edit list+card+edit amount/category/account/intent. Permanent [?? ��������] on confirmed msgs. Redis TTL 300s. ULID+workspace guards. Strict callback_data ?62 bytes verified. No search/date/delete/soft-delete/GIN, no migrations, no /balance or /report changes. 43/43 Phase 1.28 smoke + 841/841 regression smoke + 13/13 typecheck/lint = 897/897 PASS. Traceability ? Adversarial Security ? Scope Guard ?. Implementation commit c8bbc7d. Tag `phase-1.28-accepted` pushed. |
...
| 1.39 Gate UX � Edit-In-Place (Variant B) | ? DEPLOYED | `ai-parse.worker.ts` (gate block: one edit-in-place instead of 2 new messages), `screen-builder.ts` both apps (`buildGatePausedPreview`: ?? alert banner + draft summary + keyboard stays). `formatAmount()` hardened: `String()` cast ��� Postgres NUMERIC. `clarification.service.ts`: `::TEXT` cast �� `parsed_amount`. Commits `8fa8f91` > `089abf6`. |
...
| 2.2 Settings UI Overhaul | ? DEPLOYED | `settings-keyboard.service.ts` (MODIFY � 6-button 2x3 grid, URL ���������, ���� � ����), `currencies.ts` (MODIFY � Russian aliases, 5-pass search, FIAT 40+ / CRYPTO 48+), `settings.service.ts` (FIX � `deleted_at IS NULL` � `getWorkspaceAccounts`), `webhook.route.ts` (MODIFY � ������ ����� ����� ������ ������, ������ Main Account handler). Commit `3e650c1`. |
| 2.3 Search Pagination + UX Polish | ? DEPLOYED | **Pagination:** `transaction-hub.service.ts` (SEARCH_PAGE_SIZE=8, ��� 4 search-������ > LIMIT/OFFSET + COUNT(*) = `{items, total}`). `transaction-keyboard.service.ts` (`buildSearchResultsKeyboard(items, page, totalPages)` � ??/?? ����������, `search_results_page` cmd, tx:sr:p:{page} parser). `webhook.route.ts` (Redis context `midas:tx:sr:ctx:{uid}:{cid}` TTL 600s, `search_results_page` handler, ��� text intercepts > paginated API). **Reports close:** `report-keyboard.service.ts` (?? ������� = `rp:cl` �� ���� 3 �����������, type `close` � RpCallbackCmd). `webhook.route.ts` (`rp:close` handler > deleteMessage). **Keyboard order:** `screen-builder.ts` � Row 1: [?? ������][?? �����], Row 2: [?? ����������][?? ���������]. Commits `6da4464`, `049233d`, `70a5d41`. |
| 2.3 Onboarding UX Polish | ? DEPLOYED | **��� �������������� afterCreate ������:** ����� bal_input/bal_skip ����� ������������ `buildFinishOnboardKeyboard()` + `accountAddedText()`. **����� ������� `ac:fin`:** ������ �? ���������� � ������ ���� � ������ Redis, ������� ���������, ���������� ReplyKeyboard. **Backward compat:** `ac:more` � `ac:done` ����������� ��������� (������ ������ � ����). **������:** `buildStartOnboardKeyboard()` ���������: ??>??, ?>??, ??�����>??��� ��������. **��������� ����:** `ac:skip` ���� ������ ������� (USD) ���� � ������������ 0 ������. Commit `395e1f2`. Deploy `7089846c` � SUCCESS. |
| Master Roadmap Ph.1 � Keyboard Service | ? DEPLOYED | `account-onboard-keyboard.service.ts` (+478 �����): `CURRENCY_FLAGS` registry (40+ �����, �����+�������: ????RUB ????USD ?BTC ?TH TON � ��.), `getCurrencyFlag(code)`, `CURRENCY_NAMES` (���. ��������). `buildPaginatedPicker()` � ������ 2 ������� ???? (noop �� �����). `buildCurrencyPickerText(name?,isCustom?)` � 3 ����� ������. `buildFiatCurrencyPage()` + `buildCryptoCurrencyPage()` � �������� ������ + `?? ����� ������` (ac:cur:search). `searchCurrencies(query,pool)` � fuzzy+�������������� (rub/���>RUB, dollar/������>USD). `buildNoMatchText(name,type)` + `buildNoMatchKeyboard(name,backTarget)` � ����� ��������� ����� �� ����� � blockquote, 3 ������. `buildCurrencySearch*` ������ � ����������. ������� ������: `FIAT_ITEMS`, `CRYPTO_ITEMS`, `CURRENCY_PICKER_TEXT`. Commit `35c92e0`. |
| Master Roadmap Ph.2 � Webhook FSM | ? DEPLOYED | `webhook.route.ts`: (1) `name_input` ��� fuzzy null > `buildNoMatchText`+`buildNoMatchKeyboard`, ��� `name_confirm_custom`. (2) `ac:cus:save` > `pendingName` ��� `isCustomName=true` > `cur_pick`. (3) `ac:cus:keep` > `name_input` retry. (4) `ac:cur:search` > `cur_search` ��� + ��������� ������. (5) `ac:cur:list` > ������� � ��������������� ������. (6) `cur_search` text interceptor > `searchCurrencies` > ����������/no-results. (7) 3 success-screens: `{ inline_keyboard: [] }` (��� ������). `chooseCurKeyboard()` � module-level helper. ��� callback_data ?64 ����. Commit `35c92e0`. |
...
| 2.9+ Smart Nav Message (midas:nav: key) | ? DEPLOYED | `active-message.service.ts` (������ ����������� `sendNavMessage()` � edit-first ����� `midas:nav:`, �� ������� `midas:am:`; ����� ������� `getNavMessageId`, `setNavMessageId`, `clearNavMessageId`). `webhook.route.ts` (������ 2 ����� �������; AI-parse path � cleanup `midas:nav:` ����� ����������� `midas:am:` cleanup; `st:cancel` � silently deletes ������ ��������������; `bl:close` � �������� `clearNavMessageId`). Commits `4baac9c` > `004966f`. |
| 2.10 Transaction UI Persistence & Navigation Fixes | ? DEPLOYED | **��� ����������� �����:** (1) `notifications.worker.ts` + `confirmation.worker.ts` + `shared/index.ts` � `isSuccessCard?: boolean` ����; ��� approve DEL `midas:am:` ������ SET (commit `df15a01`). (2) `transaction-keyboard.service.ts` � `parseTxCallback`: ������ ������ `parts[4]` ��� `from` ��� `tx:d:ask` � `tx:d:yes` � �������� `:s` ��������� ����� ���� delete flow; ������ ��������� � tx:view ��������� ���������� �� success card (commit `8894b92`). (3) `notifications.worker.ts` � ������ sentinel `midas:success_card:{msgId}` (TTL 30 ����) ��� `isSuccessCard=true`; `webhook.route.ts` step-7 � ��������� `EXISTS midas:success_card:{amId}` ����� `deleteMessage` � ������� ���������� �������� (commit `b869c03`). |
| Balance Phase A � Grouped UI | ? DEPLOYED | `balance-keyboard.service.ts` (NEW: `classifyAccountGroup`, `GROUP_EMOJI`, `GROUP_ORDER`, `GroupType`, `buildBalanceListKeyboard` � emoji-���������� �� �������, `export formatBalanceShort`). `balance.service.ts` (MODIFY: ���������������� ����� ??/??/??/??/??, ����� `CURRENCY_TOTALS_SQL`). Commit `4a1748c` pushed to main. Railway auto-deploy ?. |
| Balance Phase B-1 � DB Schema | ? DEPLOYED | `packages/database/migrations/1779800000000_account-parent-and-subtype.js` (NEW). `account_sources`: `parent_account_id VARCHAR(26) FK REFERENCES account_sources(id) ON DELETE CASCADE` (NULL=top-level), `sub_type TEXT NOT NULL DEFAULT 'general'` CHECK constraint. Partial index `idx_account_sources_parent`. Applied on Railway live DB via `node-pg-migrate up --check-order false`. Commit `75156b9`. 100% �����: formula ? FK 31/31 ? defaults safe ? |
| Balance Phase B-2 � Hierarchical UI | ? DEPLOYED | `balance.service.ts` (MODIFY): `PER_ACCOUNT_SQL` + `parent_account_id`; `AccountBalanceRow` + `parentAccountId`; `getBalanceData()` builds childrenMap, renders +/L ladder for parent>children, leaf accounts unchanged. `balance-keyboard.service.ts` (MODIFY): `BalanceAccountRow` + `parentAccountId?`+`childCount?`; `BalanceCallbackCmd` + `add_currency`; `parseBalanceCallback` handles `bl:ac:{id}`; `pluralizeCurrency()` RU plural; `buildBalanceListKeyboard()` � parent aggregation (N �����) + indented child rows (L CURRENCY � balance) + ? �������� ������ (bl:ac:{parentId} ?32 bytes). tsc 0 errors. Commit `d04bcba`. |
| 2.10+ Gate Fix � Frozen UI on Concurrent Input | ? DEPLOYED | **��������:** ������������ ����� TX1 (����� ����� ������), TX2 > step-7 ������� ����� �� ���� ��� gate ��������� `gate_sent` > gate ��������� ����� ��������. TX3 > step-7 ����� ������� gate-�������� (gate_sent �� ����������) > ai-parse ������ (gate_sent ����������) > UI ��������. **���� 1:** `webhook.route.ts` step-7 ������ 5446�5458 � `EXISTS midas:gate_sent:` ����� deleteMessage; ���� ������� � �������� � `midas:am:` �� ���������. **���� 2:** `webhook.route.ts` ia:pk: ������ 1539 � `DEL midas:gate_sent:` ����� ������ ����� > ���������� flow �����������������. **���� 3:** `ai-parse.worker.ts` � gate �������������� ������ ����� ������ ����� `accountId = null`. **��������� ���� gate_sent:** SET � ai-parse > DEL ��� ia:cancel (������ 1432, �� �����) / ia:pk: (������ 1539, �����) / approve/reject � confirmation.worker (������ 268, �� �����) / TTL 1h. Commit `8d25ec1`. tsc 0 ������. Railway ? ��� ������� Online. |

---

## 3. �������� ������������� �������

- **Runtime:** Node.js 24 + TypeScript (ADR-001). Python � ������ ������������� ����������� �����.
- **Frontend (future):** React 19 + Vite 8. Vue ������� (ADR-002).
- **Workspace:** MVP = 1 default workspace �� ������������. �� multi-workspace-ready � ������� ��� (ADR-003).
- **Auth:** WorkspaceMembership required. Telegram User ID = �������������.
- **Primary Keys:** ULID (ADR-004).
- **DB Isolation:** PostgreSQL RLS. Low-privilege DB role. `BYPASSRLS` ��������.
- **Tenant Context:** `SET LOCAL app.workspace_id` ������ ������ `withTenantTransaction(workspaceId, fn)` (SEC-03).
- **Queue:** BullMQ (Redis-backed) (ADR-014).
- **Financial Precision:** Decimal / NUMERIC only. `Number`, `parseFloat`, `Number()`, float arithmetic ��������� (SEC-02).
- **AI Output:** Strict Zod allowlist. AI �� ����� ����������/�������������� ��������� ���� (SEC-01).
- **Draft Lifecycle:** TransactionDraft > pending_user > approved/rejected/expired/needs_clarification.
- **Security:** SEC-01 � SEC-12 ����������� ��� Phase 1.
- **AI Pipeline (claude-client.ts + prompts.ts):**
  - ������: `claude-haiku-4-5`, `temperature: 0` (�����������), `max_tokens: 256`
  - System prompt: OUTPUT RULES > MULTILINGUAL RECOGNITION (RU/EN/UA) > FUZZY MATCHING (��������, �����, ��������������) > BILINGUAL PAIRS (����������� ��������) > DISAMBIGUATION RULES (15 ������ ��� ������������� �������) > COMPOUND EXPRESSIONS > DEFAULT INTENT PRIORITY > 30-������������ ���������� (18 personal + 12 business) ? 500+ ������� �������/�����/������� (���/EU/US) > RUSSIAN LANGUAGE RULES (50+ �������� �������/������) > CATEGORY>INTENT defaults > 25+ �������� (��� 5 intent-����� + partial + nonsense)
  - Markdown fence strip: Claude ������ ����������� JSON � ` ```json `, ������ ��� ������� ����� `JSON.parse`
  - Zod validation: strict allowlist � intent/amount/currency/category_hint/person_hint/account_hint/item_hint/note/confidence
  - **Category validation (Phase 1.37):** `ALLOWED_CATEGORIES` Set � ���� Claude ������ `category_hint` �� �� ����������� ������, ���������� �� `������`
  - Post-processing (safety net, ����� Claude): 7 ����� regex � word-boundary `\b`, negation guard, confidence boost (+0.15/+0.25), intent fallback
  - ���������: `ok` | `partial` (missing fields) | `needs_clarification` (nonsense) | `rejected`
  - **Phase 1.35:** `item_hint` (extracted product/merchant name), `category_hint` (AI category suggestion) > `CategoryResolverService` (3-stage: exact > 200+ alias map > fallback �������)
  - **Phase 1.37:** Zero-clutter UX, ������������� ����������, �������������, ������� ��������� ���������
...
- **UX Architecture (Phase 1.33�1.36-UX) � ��������� ������� ���������:**
  ...
  - Post-confirm card: `[?? �������� ������]` only � nav buttons removed (handled by Reply Keyboard)
  - **Persistent Navigation:** `ReplyKeyboardMarkup` (`is_persistent: false`, `resize_keyboard: true`) � 2?2 grid: Row 1 `[?? ������][?? �����]`, Row 2 `[?? ����������][?? ���������]`. Sent on `/start`. NAV_BTN_* intercepted before AI parse. **(Phase 2.3: ����� � ���������� �������� ������� � ����� ������ ������ ������)**
  - **Transaction Hub Filter Row (Variant D � ���������):** 1 ������ ? 5 ��������� ������-�����: `[??]` ������� � `[??]` ������ � `[??]` ����� (merged) � `[??]` �������� � `[?? ���]`. �������� ������: `emoji + ' ?'`. ������� �� �������� (�� ����) ������� ������ > ������� � 'a'. SQL: `'d' AND intent IN ('debt_given','debt_received')`. ���������: `[?? �����]  [?? X / Y]  [������ ??]`. IntentFilter: `'a' | 'e' | 'i' | 'd' | 't'`. Backward compat: `dg`/`dr` > `'d'`.
  - **Keyboard Carrier:** Greeting message `? �� ��� ����������������...` ������� � ���� **��������** � �������� ���������� ��������� ReplyKeyboardMarkup. �� ��������� �� ��� ����� ��������.
  - **Transaction History (FINAL):** ������ preview-�������� � ��� **�����** ��������� (`sendMessage`), `activeMessageId` �� ��������� �� `ai-parse.worker`. ������� ���������� ������������� � ����. ������ �������� `midas:am:{userId}:{chatId}` (active-message pointer) **�����** �� notifications.worker.
  - **Preview>Confirmed Edit:** ��� approve `confirmation.worker` ������ `midas:preview:{draftId}` (TTL 600s) � message_id preview-��������, ���������� `notifications.worker` ��� ��������. Approve > `editMessageText(previewMsgId, confirmedText, inlineKeyboard)`. Reject > `editMessageText(previewMsgId, ? ��������)` (Phase 1.38 fix).
  - **Redis Keys (����������):**
    - `midas:preview:{draftId}` � message_id preview-��������, TTL 600s. ���������� notifications.worker. ������ � ������� confirmation.worker �� approve � reject.
    - `midas:greet:{userId}:{chatId}` � ����������� � /start handler, �� ������� �� ������������ ��� �������� (��� �������� ��� ��������, ���������).
    - `midas:clar:{userId}:{chatId}` � intercept ��� ����� ����� ��� clarification. ��������� �� confirm/reject (race condition fix).
    - `midas:clar:msg:{userId}:{chatId}` � message_id nonsense-���������. ��������� ��� ��������� �������� �����.
    - `midas:ac:{userId}:{chatId}` � account onboarding state, TTL 300s.
    - `midas:edit:{userId}:{chatId}` � edit amount intercept, TTL 300s.
    - `midas:awaiting_cur:{chatId}` � TTL 600s. �������� ����� ���� ����� �� ��� ������ � ��� `cur_set`. ������ `{draftId}:{workspaceId}:{userId}`. Webhook ������ ��� intercept ����� ������.
    - `midas:cur_set:{workspaceId}` � ���� ����, ��� ������������ ��������� ������� ������ � ����������. ���� ���� � ������ �� �������������.
    - `midas:gate_sent:{telegramUserId}:{chatId}` � ���� ��� gate ��� �������� (TTL 1h). ������������� ��������� edit ��� ������ ����� ���������.
    - `midas:dead_card:{chatId}` � message_id �������� "? ��������" ��� "? �������� ����", TTL 24h. ������������ confirmation.worker (reject/expired) � draft-expiration.worker (CRON expire). �������� � ��������� ai-parse.worker ��� �������� ��������� preview � �������� ������������� ��������� �� ����. (Phase 1.40)
     - `midas:am:{userId}:{chatId}` � Phase 2.10: pointer �� ������� �������� ��������� (���������, ������ ������, clarification). TTL 24h. ��� approve ���������� � DEL (�� SET, ����� success card �� ���������). Step-7 � webhook.route.ts ��������� `midas:success_card:{amId}` ����� ���������.
     - `midas:success_card:{msgId}` � Phase 2.10: sentinel key, TTL 30 ����. ������������ `notifications.worker` ��� `isSuccessCard=true` (����� approve). �������� step-7 � `webhook.route.ts` � ���� EXISTS, ��������� �� ��������� ��� ����� ��������� ����������. ������� ���������� ������ � DEL `midas:am:`.
    - `bl:state:{telegramUserId}:{chatId}` � Phase 2.1: state ��� ��������� intercepts ������-�����������. ������ `{action, accountId}`. Actions: `rename`, `set_balance`, `currency_input`. TTL 300s.
    - `bl:source:{telegramUserId}:{chatId}` � Phase 2.1: ���� ��� ���������� ����� ������������ �� �������. ��� `ac:done` ���������� � balance dashboard ������ setup complete.
     - `midas:tx:sr:ctx:{telegramUserId}:{chatId}` � Phase 2.3: ��������� �������� ��� ���������. ������ JSON `{t: 'name'|'amount'|'category'|'date', q?: string, f?: string, to?: string, lb?: string}` TTL 600s. �������� ��� ������ ������, �������� ��� ��������� �� ��������� (tx:sr:p:{page}). ��� ����������� � ����������� ��������� ������ ������.
    - `bl:source:{telegramUserId}:{chatId}` � Phase 2.3: ��� `ac:fin`/`ac:done` ����������� ��� �������� � balance dashboard ������ setup complete.
     - `midas:ac:{userId}:{chatId}` ���� `pendingName` � Master Roadmap: ��������� ��� �� no-match flow �� ������������� ����� `ac:cus:save`.
     - `midas:ac:{userId}:{chatId}` ���� `isCustomName` � Master Roadmap: true ���� ��� ����� � ��������� ���� (�� preset). ������ �� ����� currency picker.
     - `midas:ac:{userId}:{chatId}` ��� `cur_search` � Master Roadmap: ����� ������ ������ �������. ��������� ����� ������������ > `searchCurrencies()`. ��������� ��� `ac:cur:list` ��� ������ ������.

  - **Auto-Activation:** `replyKeyboardJson` � `NotificationJobPayload`. rejection/expiry/intent_missing sends ReplyKeyboard �� `sendMessage` path. `editMessageText` path � ������ inline keyboard (Telegram API limitation).
  - **Collapsibility:** `is_persistent: false` � Telegram ���������� ? ������ ����� � ??; ������������ ����� ��������/��������������� ����������.
  - **Race Condition Fix:** `redisConnection.del(clarKey)` �� confirm/reject > stale `midas:clar:*` �� ������������� ��������� ���������.
...
- `project_config.md` ������ **v1.4**
- v1.4 ��������: Phase 1.37 AI Taxonomy & Zero-Clutter UX update, 30-category taxonomy, 500+ anchors, multilingual, disambiguation, ALLOWED_CATEGORIES validation, Phase 2.0 documented
- SEC-01 � SEC-12 = ������������ ����������� ���������� Phase 1
- **?? ������������** � ��������� ������ �� ������� ������� ���������
...
������ Turborepo monorepo `midas-monorepo/`:
...
+-- packages/
## 6. ������� ���� � PHASE 1.30: Smart Account Onboarding

> ? **COMPLETED / ACCEPTED (Phase 1.30). See Section 10 history.**

**Objective:**
Replace the flat "������ ���� ���." empty-state with a guided interactive keyboard when /accounts is empty (Scenario �) and show a guided account setup keyboard for new users after /start (Scenario �). UX layer only � no migration, no new commands, no AI changes.
...
- `[? ����������]` button on /start guided keyboard � clears state, no account created.
- Cash auto-name: "�������� {CURRENCY}" derived at creation time.
- Exchange presets: Binance, Bybit, OKX, Kraken, Huobi + ?? ������.
...
| MCP-������ | ������ | ���������� |
|---|---|---|
| **Railway MCP** | ? Active | ������, ����, ����������, �������. Project: `spirited-happiness`. |
| **GitHub MCP** | ? Active | Repo: `gloryjasystem/Midas`. Auto-deploy on push to `main`. |
| **Postgres MCP** | ? Active | Read-only SQL � production DB ����� Railway proxy. |
| **Filesystem MCP** | ? Active | ������/������ ������ � workspace `C:\Users\secvency\Desktop\Midas` |
...
| `TELEGRAM_BOT_TOKEN` | Midas | ?? ������� ������� (��� ����� � �����) |
| `ANTHROPIC_API_KEY` | background-workers | ?? ������� ������� |
| `TELEGRAM_WEBHOOK_SECRET` | Midas | `midas_wh_secret_2026_prod` |

---

## 8. ����� ��� ������ � ����� ���� (Phase Balance-B-2 context)

**? ������� ��������: Balance Redesign Phase A ? + B-1 ?. ��������� � Phase B-2 (������� � getBalanceData + ��������� �������� ������).**

**����������� ��������� � ����� ����:**
```
apps/telegram-bot/src/services/balance-keyboard.service.ts   < �����������, buildBalanceListKeyboard
apps/telegram-bot/src/services/balance.service.ts            < PER_ACCOUNT_SQL, getBalanceData
packages/database/migrations/1779800000000_account-parent-and-subtype.js < ����� B-1
```
...
**��������� �� (��������� SQL-�������):**
- `parent_account_id` � ��� NULL (��� ����� top-level, �������� ��� �� ���������)
- `sub_type` � ��� `'general'` (Phase A ���������� ��������� `classifyAccountGroup`)
- 31 ����������, ������� ������� ��������� (initial_balance + income ? expense)
...
```
? Balance Redesign � Phase A ? + Phase B-1 ? ���������. ��������� � Phase B-2.

������:
Midas Telegram Bot. Railway (project: spirited-happiness). MCP: Railway, GitHub, Postgres, Filesystem.
Auto-deploy: push to main > GitHub > Railway ������ Midas + background-workers.

��� ��� �������:

Phase A (commit 4a1748c, ���������):
- balance-keyboard.service.ts: classifyAccountGroup(��������� �� name/currency),
  GROUP_EMOJI (??/??/??/??/??), buildBalanceListKeyboard � ��������, export formatBalanceShort
- balance.service.ts: ���������������� ����� getBalanceData(), ����� CURRENCY_TOTALS_SQL

Phase B-1 (commit 75156b9, ��������� �� live Railway Postgres):
- migration 1779800000000_account-parent-and-subtype.js:
  parent_account_id VARCHAR(26) FK (NULL=top-level ����)
...
- FK-�����������: 31/31 ���������� ������� � ������������� �������
- draft-confirmation.service.ts (transaction INSERT) � �� �������� ������ �����������

��� ����� ������� (���� B-2):

1. balance.service.ts � �������� PER_ACCOUNT_SQL:
   - �������� parent_account_id � SELECT
   - ������� ������ � getBalanceData(): parent ����� + ��������� children
   - �������� ����������� � ��������: + OKX USDT � 32 601 / L OKX BTC � 0.5

2. balance-keyboard.service.ts � �������� buildBalanceListKeyboard:
   - Parent-����: ���������� ��������� ("���������� �����")
   - Child-����: ������ + ������ ������
   - �����: ������ "? �������� ������" (bl:ac:{parentId})

�������� �������:
- ���������� ����������: ������ BigInt/NUMERIC, ������� float (SEC-02)
- ��� ������� ����� withTenantTransaction (SEC-03)
- �� ������� project_config.md
- �� ������ draft-confirmation.service.ts � ���������� �������� ��������

����������� ��������� workflow_state.md ������ 16 (�������) � ���� B-2 ����.
```
...
| ���� | ������� |
|---|---|
| 2026-05-04 14:07 | ������������� �������: project_config.md v1.0 + workflow_state.md |
| 2026-05-04 14:45 | Phase 0.1 Event Storming completed (46 �������, 10 ���������, 15 ADR planned) |
...
| 2026-05-04 17:02 | Security review: 2 CRITICAL, 2 HIGH > Phase 0.3.1 �������� |
...
| 2026-05-09 09:46 | Phase 1.36-UX Sub-step 1: Persistent Navigation Keyboard (core). `telegram-api.ts` � `ReplyKeyboardMarkup` interface + `sendMessageWithReplyKeyboard()`. `screen-builder.ts` (telegram-bot) � `buildMainMenuKeyboard()`, `NAV_BTN_BALANCE/REPORT/SETTINGS`, `input_field_placeholder`. `webhook.route.ts` � Reply Keyboard sent on /start (new+existing users), 3 text intercepts before AI parse for [?? ������]/[?? �����]/[?? ���������] buttons. Collateral lint: `ai-core/claude-client.ts` (no-useless-assignment), `draft-confirmation.service.ts` (no-unnecessary-type-conversion ?3), both `screen-builder.ts` (restrict-template-expressions). 13/13 PASS. |
| 2026-05-09 10:00 | Phase 1.36-UX Sub-step 2: UX Bug Fixes & Consistency. (1) `webhook.route.ts` confirmKb layout standardized: ? full-width top row + [?? ��������|?? ������] split row � matches workers layout. (2) `redisConnection.del(clarKey)` added on approve/reject in `webhook.route.ts` � prevents stale `midas:clar:*` key intercepting next user message after confirmation (silent message discard race condition fixed). (3) `screen-builder.ts` both apps � emoji ?>?? for visual weight parity with ? and ??. 13/13 PASS. Commit `c2f012f`. |
| 2026-05-09 10:12 | Phase 1.36-UX Sub-step 3: Reply Keyboard auto-activation. `shared/index.ts` � `replyKeyboardJson?` added to `NotificationJobPayload` (documented: only valid on sendMessage, not editMessageText). `background-workers/screen-builder.ts` � `buildNavKeyboard()` replaced by `buildMainMenuReplyKeyboard()` (returns plain JS object with `keyboard` array, not InlineKeyboard); `buildPostConfirmKeyboard()` nav row [?? ������][?? �����] removed � only [?? �������� ������] remains. `confirmation.worker.ts` � import updated (buildNavKeyboard>buildMainMenuReplyKeyboard); rejected/expired/intent_missing now pass `replyKeyboardJson` (not `inlineKeyboardJson`). `notifications.worker.ts` � keyboard routing split: `inlineReplyMarkup` for editMessageText path, `freshReplyMarkup` (prefers replyKeyboardJson) for sendMessage path. Reply Keyboard auto-activates on first new message without /start. 13/13 PASS. Commit `f10aa22`. |
...
| 2026-05-09 12:57 | Phase 1.36-UX FINAL (accepted): Transaction history workflow + permanent keyboard. **��������:** edit-first ��������� ����� `midas:am:` pointer �������������� ���������� �������� ������ �������� ����� � ������� ���������� �� �������������. **�������:** (1) `ai-parse.worker.ts` � ����� `activeMessageId` �� preview notifications; ������ preview-�������� ������ ������������ ��� ����� ���������. (2) `notifications.worker.ts` � ��� �������� preview (draftId ������������) ���������� `sentMessageId` � Redis `midas:preview:{draftId}` TTL 600s; ����� `setActiveMessagePointer` � ���� AM-pointer ��������. (3) `confirmation.worker.ts` � �� approve ������ `midas:preview:{draftId}` > ������� ��� `activeMessageId` � notifications (edit preview>confirmed in-place); �� reject � `activeMessageId` �� ��������� > ����� ���������. (4) Greeting: �� ��������� � ������� ���������� ��������� ReplyKeyboard; ���� ��� �������� (deleteMessage + nav carrier) �����. `greetingMsgId` ����� �� `NotificationJobPayload`. `shared` ����������. Typecheck 0 errors (��� ����������). Commits `e879dfc` > `2cb86c4` > `8941c6d` > `2a15f31`. Deployed to Railway. ��������������: 4 ���������� ��������, ������� �������������, ���������� [?? ������][?? �����][?? ���������] ��������� �����. |
| 2026-05-09 13:09 | Phase 1.37 Step 1: Zero-clutter UX. `screen-builder.ts` (background-workers): `buildNonsenseScreen()` rewritten � removed all inline buttons ([?? ������][?? �����][?? ���� ���][?? ���� ����]), replaced with Variant 5 text-only prompt with input examples (`���� 150 UAH � �������� 5000 USDT`). `ai-parse.worker.ts`: added stale "�� �����" message deletion � stores `midas:clar:msg:{userId}:{chatId}` Redis key pointing to nonsense message_id; on next successful parse, deletes the old nonsense message via `deleteMessage()` API before sending new preview. `telegram-api.ts`: `editTelegramMessage()` � treats "message is not modified" 400 error as success (no redundant message generation). Typecheck 8/8 PASS. Commits `a4d49a9` > `ee85e5f`. |
| 2026-05-09 13:34 | Phase 1.37 Step 2: Category taxonomy expansion. `prompts.ts`: Expanded from 28 to 30 categories (added �������, ���). International 500+ anchor items mapping: every category now has typical items across CIS (��������, ���, ѳ����), EU (Lidl, Biedronka, IKEA), US (Walmart, Costco, Amazon, Starbucks) markets. Business categories expanded with global services: AWS, Stripe, Upwork, Fiverr, Google Ads, Facebook Ads, Notion, Figma, etc. Pet category: Royal Canin, Whiskas, Pro Plan, �����������, ���������. ���: ������ ��������, ������, ���������, �����, ������. Typecheck 8/8 PASS. Commits `77a0ad9` > `5b02cf3`. |
| 2026-05-09 14:09 | Phase 1.37 Step 3: Multilingual recognition + fuzzy matching. `prompts.ts`: Added MULTILINGUAL RECOGNITION section (RU/EN/UA � any language maps to correct category). FUZZY MATCHING section (typos: ����>����, ��������>Netflix; slang: ���������>����������>�����; transliteration: kafe>����, taksi>�����). KEY BILINGUAL PAIRS for non-obvious translations (����������=tire service, ���������=payment processing, ����������=diapers, �����������=cat litter, etc.). Commit `e147240`. |
| 2026-05-09 14:10 | Phase 1.37 Step 4: Disambiguation rules + compound expressions + default intent priority. `prompts.ts`: Added 15 DISAMBIGUATION RULES (����>��������/�������/���� by context; ����>����/��������; ���������>���������/��������/�����������; ������>�����/���������/������������; ��������>��������/�������; etc.). COMPOUND EXPRESSIONS (������� ����>�������, ���� ��� ����>�������, ����� � ����>�����������). DEFAULT INTENT PRIORITY (item+amount without verb = expense by default; income/transfer require explicit signal). Commit `03981d7`. |
| 2026-05-09 14:14 | Phase 1.37 Step 5: ALLOWED_CATEGORIES code validation. `claude-client.ts`: Added `ALLOWED_CATEGORIES` Set (30 categories � 18 personal + 12 business). Post-Zod validation step: if `aiData.category_hint` is not in the set, replace with `������`. Prevents hallucinated categories from reaching CategoryResolverService. Typecheck 8/8 PASS. |
| 2026-05-09 14:16 | Phase 1.37 Step 6: Documentation updates. `product-roadmap.md`: Added Phase 2.0 � AI Intelligence Evolution (3 components: 2.0-A self-learning from user edits, 2.0-B custom category recognition, 2.0-C regional bias from currency). Phase 1.37 + 2.0 added to summary table. Block 4 renamed from "����� � Vision" to "AI Intelligence � Voice". `project_config.md`: Updated to v1.4, changelog v1.4 added, Section 2.8 AI Pipeline updated with multilingual/disambiguation/validation info. Commit `06bccb0`. Deployed to Railway. |
...
| 2026-05-09 15:38 | Phase 1.37 VERIFICATION & ACCEPTANCE. 13/13 typecheck+lint PASS. CategoryResolver: �������/��� aliases added. Commit `641ad26`. Deployed to Railway. |
| 2026-05-09 19:00 | **Phase 1.38 Fix #1:** Confirmation card not deleted on Cancel. `confirmation.worker.ts` reads `midas:preview:{draftId}` on both approve and reject paths � in-place edit to ? ��������. |
...
| 2026-05-09 19:05 | **Phase 1.38 Fix #3:** `amt+cur` handler used `validateCurrencyCode()` (ISO-only) instead of `normalizeCurrencyInput()`. Fixed. `awaiting_cur` now extracts currency token from mixed input (e.g. �50 ����). Commit `d59025f`. |
| 2026-05-09 19:18 | **Phase 1.38 Rollback:** PRICE vs QUANTITY AI prompt rule reverted. Caused regressions (�150 ������ not extracted as amount). Design decision: personal finance bots ALWAYS treat any number as a price. Original rule restored: �If ANY number present > ALWAYS extract as amount�. Final commit `c59f2e1`. |
| 2026-05-10 10:08 | **Phase 1.39 � Gate UX Edit-In-Place (Variant B).** `formatAmount()` � ����� screen-builder.ts ���������: `String()` cast ��� Postgres NUMERIC ���� � ������� TypeError (`raw.includes is not a function`). `clarification.service.ts`: `::TEXT` cast �� `parsed_amount` � 2 SQL-��������. `buildGatePausedPreview()` �������: ?? �����-������ + summary ��������� (������ ������� ������ ��� ������). ���� gate � `ai-parse.worker.ts` �����������: ������ 2 ����� ��������� (paused edit + gate card) � **����** edit-in-place ������������ preview-�������� � ������� � ����������� ���������� �������������. Commits `8fa8f91` > `089abf6`. Deployed to Railway � SUCCESS. |
| 2026-05-10 10:30 | **Phase 1.40 � Dead Card Auto-Cleanup.** ������: �������� �? �������� � �? �������� ���� ������������� ��������� �� ���� ����� ���������� ��������� preview-��������. � ���� �������� ������: pending (��� �������������) + approved (? ��������). ����������: `confirmation.worker.ts` � ����� reject/expired ��������� `previewMsgId` � Redis `midas:dead_card:{chatId}` TTL 24h. `draft-expiration.worker.ts` � CRON expiry ���� ����� dead_card. `ai-parse.worker.ts` � ����� ��������� ����� preview ������ dead_card, ������� ��� `deleteMessageId`, ������� ����. ���� ������������ ���� dead_card � clar_msg � ��������� � dead_card. TypeScript: 0 ������. Commit `51eaf10`. Deployed to Railway � SUCCESS. |
| 2026-05-10 15:30 | **Phase 2.0 � Transaction Hub + Reports 2.0 + Settings 2.0 deployed.** GitHub auto-deploy from `main`. |
| 2026-05-10 18:44 | **Phase 2.1 � Account Management Dashboard.** ������ ���������� �������������� ���������� ������� ����� ������. **����� �����:** `balance-keyboard.service.ts` (450+ ����� � parseBalanceCallback, buildBalanceListKeyboard, buildAccountActionsKeyboard, buildDeleteConfirmKeyboard, buildCurrencyWarningKeyboard, buildBalanceFiatCurrencyKeyboard, formatAccountDetailText, BalanceAccountRow type). **���������������� �����:** (1) `account-onboard-keyboard.service.ts` � �������� ��������� ������ (10: ��������, ��������, �����, ���, ����, ������, �����, N26, Revolut, Wise) � ��������� (9: Trust Wallet, MetaMask, Exodus, Ledger, Trezor, Phantom, Coinbase Wallet, SafePal, Tangem). (2) `account.service.ts` � `renameAccount()`, `changeAccountCurrency()`, `softDeleteAccount()`. (3) `balance.service.ts` � `getBalanceData()`, `getAccountDetail()`, `setAccountBalanceById()`, `getAccountTxCount()`. (4) `webhook.route.ts` � bl: callback handler, text intercepts, ac:done ��������� bl:source. **DB Migration:** updated_at + deleted_at �� account_sources. Build+Deploy: 0 ������. |
| 2026-05-10 19:58 | **Phase 2.2 � Settings UI Overhaul (DEPLOYED).** (1) `currencies.ts`: �������� ������ (FIAT 40+, CRYPTO 48+); `CURRENCY_RU_ALIASES` � 50+ ������� ������� (�������, ������, ����, �����, ������, �����, ���� � ��.); `searchCurrencies()` � 5-pass �������� (exact/startsWith/includes/EN-name/RU-alias), ����� 10. (2) `settings.service.ts`: `getWorkspaceAccounts()` + `deleted_at IS NULL` (soft-deleted ����� �� ������������); `setDefaultAccount()` �������� ��������� ��� ���� (expense+income). (3) `settings-keyboard.service.ts`: `buildSettingsMainKeyboard()` � ������� 2x3 ����; ����� ������ � �����������; ����� ����� ������ �������� ������. (4) `webhook.route.ts`: ����� ������ ������ ������ `[?? ����� � ���������]`; ������ ���������� `st:da:sa:` � ���� Main Account ��� income+expense. Build: `tsc` 0 ������. Commit `3e650c1`. Deployed to Railway (auto-deploy). |
| 2026-05-10 22:00 | **Phase 2.3 � Paginated Transaction Search.** `transaction-hub.service.ts`: �������� `SEARCH_PAGE_SIZE=8`; ��� 4 search-������� (`searchByName`, `searchByAmount`, `searchByCategory`, `searchByDateRange`) ������������ � ��������� `page: number`, ������������ `COUNT(*)` > ���������� `{items: TxListItem[], total: number}`. ������� ��������� `SEARCH_LIMIT=200`. `transaction-keyboard.service.ts`: `buildSearchResultsKeyboard(items, page, totalPages)` � ������ ������� + ������ ��������� `[??][p/total][??]` + footer `[?? ����� �����][?? � ������]`; `search_results_page` � `TxCallbackCmd`; ������ `tx:sr:p:{page}`. `webhook.route.ts`: ��� search-handlers ��������� �������� � Redis `midas:tx:sr:ctx:{uid}:{cid}` TTL 600s; `search_results_page` handler � ������ ��������, ������������� offset, ��������� ���������; text intercepts (name/amount/date) > paginated API; ��� ���������� ��������� � ����������� �������� �����; ����� ����������� ������ text intercept ����. Build: `tsc` 0 ������. Commit `6da4464`. |
| 2026-05-10 22:10 | **Phase 2.3 � Reports Close Button.** `report-keyboard.service.ts`: �������� `rp:cl` callback (`?? �������`) ��� ��������� ������ �� ���� 3 ����������� (`buildPeriodPickerKeyboard`, `buildReportSubMenuKeyboard`, `buildReportBackKeyboard`); ��� `{ cmd: 'close' }` �������� � `RpCallbackCmd`; `parseRpCallback`: `rp:cl > { cmd: 'close' }`; ������� docstring. `webhook.route.ts`: � ����� `rp:` �������� handler `else if (rpCmd.cmd === 'close')` > `deleteMessage(chatId, rpMsgId)` � ��������� ������� ��������� �� ����. Build: `tsc` 0 ������. Commit `049233d`. |
| 2026-05-10 22:11 | **Phase 2.3 � Persistent Keyboard Button Order.** `screen-builder.ts` (`buildMainMenuKeyboard`): ������� ������ ������ � Row 1: `[?? ������][?? �����]`, Row 2: `[?? ����������][?? ���������]` (��: Row 1 ������+����������, Row 2 �����+���������). ������� docstring. Build: `tsc` 0 ������. Commit `70a5d41`. Deployed to Railway (auto-deploy). |
| 2026-05-11 09:00 | **Phase 2.2 Onboarding Pagination (Phase 2.2).** `account-onboard-keyboard.service.ts` ��������� ��������� � ������������� ������� ��������� `buildPaginatedPicker()`. �����������: paginated banks (70+ �������, 6/��������, 3 �������, ac:bp:{N}), paginated exchanges (ac:xp:{N}), paginated fiat currencies (ac:cfp:{N}), paginated crypto currencies (ac:ccp:{N}). `OnboardStep` ��������: `bal_input`. `AccountOnboardState` � ���� `accountId`, `currency`. `addAccountReturningId()` �������� � `account.service.ts`. `webhook.route.ts`: FSM handlers ��� bank_page, exchange_page, fiat_page, crypto_page, bal_skip. ������ �������� ������� (validateAmountFromText intercept) ��� ������������ (ac:bal:s). ������ � phase 2.2 �����. tsc 0 ������. |
| 2026-05-11 12:00 | **Phase 2.3 Onboarding UX Polish (PLAN APPROVED).** ������������ �������� ����: (1) ������ ������������� afterCreate �����, (2) �������� ������ �? ���������� (ac:fin) ����� � ����� ����, (3) buildStartOnboardKeyboard � ��������� ������ (??>??, ?>??), (4) ��� �?? ������ ��� ����� ���� ��������� ������� (USD). ���������� �������� �� 4 ����� � tsc-��������� ����� �������. |
| 2026-05-11 14:07 | **Phase 2.3 Onboarding UX Polish � ���� 1 (account-onboard-keyboard.service.ts).** �������� `{ cmd: 'fin' }` � `AccountOnboardCmd` union + ������ `if (sub === 'fin')`. ��������� `buildFinishOnboardKeyboard()` � ����� ���� + �? ���������� (ac:fin), ������ ????. ��������� `accountAddedText(name, currency)`. `buildStartOnboardKeyboard()` ���������: ??>??, ?>??, ??�����>??��� ��������. tsc 0 ������. |
| 2026-05-11 14:08 | **Phase 2.3 Onboarding UX Polish � ���� 2 (imports).** `webhook.route.ts`: ��������� ������� `buildFinishOnboardKeyboard`, `accountAddedText` �� account-onboard-keyboard.service.js. tsc ���� 2 �������������� (unused � �������� �� ����� 3). |
| 2026-05-11 14:10 | **Phase 2.3 Onboarding UX Polish � ���� 3 (handlers).** `webhook.route.ts`: (1) `ac:fin` handler � ��������� `ac:done`, backward compat; (2) `ac:more` > redirect to fin flow (deleteMessage + sendMessageWithReplyKeyboard); (3) `ac:bal:s` � ������ ��������� Redis, ����� ���������� `accountAddedText` + `buildFinishOnboardKeyboard` ������ ������� afterCreate; (4) `bal_input` text intercept � `buildFinishOnboardKeyboard` ������ `buildAfterCreateKeyboard`, `accountAddedText` ������ ������ ������ � ��������; (5) safety fallback � `bal_input` > `buildFinishOnboardKeyboard`. tsc 0 ������. |
| 2026-05-11 14:13 | **Phase 2.3 Onboarding UX Polish � ���� 4 (default account).** `webhook.route.ts` `ac:skip` handler: ����� ��������� Redis-����� �������� `hasAccounts()` � ���� 0 ������, ������ `addAccountWithCurrency(workspaceId, userId, '������', 'USD')` � ����� try/catch (non-fatal). tsc 0 ������. Commit `395e1f2`. git push origin main. Deploy Railway: `7089846c � SUCCESS`. |
| 2026-05-11 16:30 | **master_roadmap Phase 1 � Keyboard Service.** `account-onboard-keyboard.service.ts` +478 �����: `CURRENCY_FLAGS` (40+ �����: ????RUB ????USD ?BTC ? ETH TON � ��.), `getCurrencyFlag(code)`, `CURRENCY_NAMES`. `buildPaginatedPicker()` ����������� � ��� ������� ������, noop �� �����. `buildCurrencyPickerText(name?,isCustom?)` � 3 �����. `buildFiatCurrencyPage()` + `buildCryptoCurrencyPage()` � ����� + ac:cur:search. `searchCurrencies()` � fuzzy+��������������. `buildNoMatchText/Keyboard`. `buildCurrencySearch*`. ������� FIAT_ITEMS, CRYPTO_ITEMS, CURRENCY_PICKER_TEXT. tsc 0 ������. |
| 2026-05-11 16:33 | **master_roadmap Phase 2 � Webhook FSM.** `webhook.route.ts`: `name_input` > no-match screen ��� fuzzy null. `ac:cus:save` > isCustomName=true > cur_pick. `ac:cus:keep` > name_input retry. `ac:cur:search` > cur_search step. `ac:cur:list` > ������� � ������. `cur_search` text interceptor > searchCurrencies > ���������� ��� no-results. 3 success-screens button-free `{ inline_keyboard: [] }`. `chooseCurKeyboard()` module-level. ��� callback_data ?64 ����. tsc 0 ������. |
| 2026-05-11 16:43 | **master_roadmap Phase 3 � Smoke Tests.** `smoke-test-master-roadmap.mjs` (NEW): 70 ��������, ������ `node apps/telegram-bot/smoke-test-master-roadmap.mjs` (������ ����������������� dist/). ������� ��� 14 ���������. ���������: **70/70 ? / 0 ?**. |
...
| 2026-05-12 15:05 | **workflow_state.md ��������������. ����� ��������.** `smoke-test-master-roadmap.mjs`: ��������� ���������� assert ��� `buildCurrencySearchNoResultsText`. ����: 76/76 ?. `smoke-test-lazy-default.mjs`: 39/39 ?. `tsc --noEmit`: 0 ������. Phase LD++ ��������� ������������. |
...
| 2026-05-12 17:27 | **Phase 2.4 � UX Design ������ � ������������.** ��������������: �������� + ���������� ������� (�?? Bybit USD� + �?? 15 400 ? 10 000 = 5 400 USD�), ����� ������ (������ �?? ������� ����), �����-������ (���� ����� �����������), confirmed card ��� ������ ������/�����. UX-��������� ia:list/ia:back �� �������� ���� �������� (������� ���� ���������� � stable). 16 ��������� PR ��������������. ������ ����������: 1 breaking change (PR 7 buildConfirmKeyboard), 1 ����� Redis-������� (midas:xfx:ptr). ������ ����: `account_debit_ux_plan.md`. workflow_state.md �������. |
| 2026-05-12 21:00 | **Phase 2.4 � Account Picker UX Hotfixes.** ����������� ������������ ���� ���������� ������ ��� AI parse ��� account_hint. � `ai-parse.worker.ts` �������� �������������� ����� ������. � `draft.service.ts` ��������� `getWorkspaceAccountsForPicker` ��� �������. � `draft-confirmation.service.ts` ��������� ������ (`accountWasExplicitlyChosen`) �� ����� ��������������� XFX ��� ������������ ������ ���������� �����. �������� intent-aware ������ (�����/������) ��� ������ ������ � `account-inline-keyboard.service.ts`. ��� 103/103 smoke-����� ������. |
| 2026-05-13 08:17 | **Phase 2.5 ��� 1 � Smart Item>Category Auto-Detector.** `item-category-detector.service.ts` (NEW): 200+ ������� � �������� ����, 9 ��������� (���������/���/�����������/������/��������/���/�����������/�����������/������������), longest-phrase-first matching. `patchDraftCategoryHint()` � `clarification.service.ts`: atomic idempotent DB patch (�������������� ������ ���� `parsed_category_hint IS NULL` ��� `= '������'`). ���������� � `webhook.route.ts` > `sendAndStorePreview`: non-blocking, �� ��������� flow ��� ������. ����: �������� > ���������, �starbucks� > ���. tsc 0 ������. |
| 2026-05-13 08:20 | **Phase 2.5 ��� 2 � Account-Currency Compatibility Validation Gate.** `account-currency-validator.service.ts` (NEW): ������� 8 ������, `classifyCurrency()`, `HYBRID_EWALLET_KEYS`, `TON_ASSETS`. ������������ � 2 ����� `webhook.route.ts`: (1) `cmd=currency` callback � editMessageText � �������, FSM state ����������� � Redis; (2) `cur_input` text interceptor � upsertBotMessage � �������, `redisConnection.del` �� ����������. ���������: ��������+USDT, ��������+ETH, Lightning+USDC. ���������: Bybit+USDT, Payeer+USDT (������), MetaMask+BTC. Commit `d9ad480`. tsc 0 ������. git push > Railway deployed. |
| 2026-05-13 08:24 | **Phase 2.5 ��� 3 � Anomaly Badge � �������.** `account-inline-keyboard.service.ts` (MODIFY): ������ `classifyCurrency`. `anomalyBadge(emoji, currency)` � ���������� `'?? '` ���� emoji=`??` � ������ �� ����. `buildAccountPickerV2Keyboard` �������: `??` ��� ������, `??` ��� �����, `??` ������ ��� ����+������ �������� �� ����� �����. `buildAccountPickerForDraft`: `??` ����� `anomalyBadge()` �� `accountTypeEmoji()`. Commit `f543c5e`. tsc 0 ������. git push > Railway deployed. Phase 2.5 COMPLETE. |
| 2026-05-13 15:20 | **Phase 2.7 � Account Picker Fix & Cancellation UX.** ���������� ��������� ������� `6efe173` (always show account picker), ��-�� ������� ���������� ��� ��������� ������ ��������. ������ ��������� ��� ����� �� no-match �������� �������� �� `?? ������` (`ia:cancel`). ��� ������: ������ ��������� � �� �������� �� `rejected`, ��������� in-place �������� �� "? ��������" ��� ������, ��������� ������ �� Redis. |
| 2026-05-13 15:25 | **Infrastructure � AI Token Budget Fix.** ����������, ��� ������� ���������� ��������� ������ ��-�� ���������� �������� ������ ������� Claude (`AI daily token budget exceeded: 506188 >= 500000`). ����� Railway CLI ���������� `AI_BUDGET_MAX_DAILY_TOKENS` ��� `background-workers` ��������� � 500 000 �� 2 000 000. ������� �����������, ��������� ���������� �������������. |
| 2026-05-13 21:30 | **Phase 2.8 � ���� 1: Callback Fix (ia:newac).** `account-inline-keyboard.service.ts`: ��������� ����������� ��� � ������ �? ������� ���� � ������ ��������� �������� `ia:rename` ������ ����������� `ia:newac`. �������� ��� `showpicker` � `InlineAccountCmd` union � ��������������� ������ ��� �������� ���������. |
| 2026-05-13 21:35 | **Phase 2.8 � ���� 2: �������������� ������ ����������.** `webhook.route.ts`: ��������� ������ ������ ���� ����� (����������� ����� `ia:newac`) ������ � ����� ������������ ������ �� ��������� `ACCOUNTS_EMPTY_TEXT` � ������������� ����� ������ `/start` ��� ����� �������������. |
| 2026-05-13 21:45 | **Phase 2.8 � ���� 3: Back Navigation (ia:showpicker).** `account-onboard-keyboard.service.ts`: ������ �?? ����� �� ������ ������ ���� ����� ������ ���������� callback `ia:showpicker` ������ `ia:pk:back`. `webhook.route.ts`: ���������� ����� handler `ia:showpicker` � ��������������� `midas:prev_acct` (������������ accountId �� Redis), �������� Account Picker V2 � ����������� `linkedDraftId`. ������������ ����� ��������� � ������ ��� ������ ��������� ���������. |
| 2026-05-13 22:00 | **Phase 2.8 � ���� 4: �������� success-��������.** `webhook.route.ts`: ������� ������ �? ���� ... ������!� �� ���� ��� ����� ���������� ���������� �� ���������: `bal_skip`, `bal_input`, `cur_input`. ������ ����� �������� ����� ����� ������������ preview-�������� ��������� ����� `confirmPreviewFull()` � ������ seamless UX ��� ������������� �������. |
| 2026-05-13 22:30 | **Phase 2.8 � TS Build Fix.** ���������� ������ ������ �� Railway: `TS6133: 'linkedAccountNameBal' / 'acNameBi2' is declared but its value is never read` � ���������� ����� ��������������� ����� �������� success-�������� � ����� 4. ������� ��� ����������. `tsc --noEmit`: 0 ������. Commit `56991be` pushed to main. Railway re-deploy: Midas ? Online. |
| 2026-05-14 10:43 | **Phase 2.9 � Nav Buttons Never Delete Tx Records.** ��������: ����� �������� ���������� � message_id (�? �������� + �?? �������� �������) �������� � Redis ��� `midas:am:` pointer. ��� ������� ������/�����/����������/��������� � `upsertBotMessage()` ������������ ��� ������ ��� ���������. �������: �������� `sendNavMessage()` � `active-message.service.ts` � ������ ���������� ����� ���������, �� ������� `midas:am:`. 4 NAV_BTN_* ����������� � `webhook.route.ts` ����������� �� `sendNavMessage`. Commit `1477f55` pushed to main. |
| 2026-05-14 10:57 | **Phase 2.9+ � Smart Nav Message (�idas:nav: key).** ��������: ������ ������� nav-������ ���������� ����� ��������� (��������� ����). �������: ��� ����������� Redis-�����. `midas:am:` � ���������/������/������������� (�� ������� � nav). `midas:nav:` � nav-������ (������/�����/etc.). `sendNavMessage()` ��������� ���������: edit-first ����� `midas:nav:`, ��� ������ � ����������� �� �� ��������� (��� �� ����������), ��� ������� � ���������� �����. ��� ����� ����������: `getNavMessageId` > `deleteMessage` > `clearNavMessageId` ����� ����������� cleanup `midas:am:`. Commits `4baac9c`. |
| 2026-05-14 11:04 | **Phase 2.9+ � Silent Close Button.** ������ �? �������� � ���������� (`st:cancel`) ����� ������������� ��������� �� �?? ��������� �������.� (������). ������: `deleteMessage(chatId, messageId)` + `clearNavMessageId()` � ������ ������ ��������, �������� ������ ������. ������ �? �������� � ������� (`bl:close`) ��� ������� ���������, �� �� ������� `midas:nav:` � ����������. Commit `004966f` pushed to main. Railway auto-deploy triggered. |
| 2026-05-14 12:28 | **Phase 2.10 � Fix 1: isSuccessCard � DEL midas:am: ��� ������������� ����������.** ��������: ����� ������������� ���������� success card ����������� � `midas:am:` pointer. ��� ����� ��������� ���������� step-7 � webhook.route.ts ������ ��������� �� `midas:am:` � success card ���������. �������: `shared/index.ts` � �������� `isSuccessCard?: boolean` � `NotificationJobPayload`. `confirmation.worker.ts` � ��� approve: `isSuccessCard: true` � payload. `notifications.worker.ts` � ���� `isSuccessCard`: `DEL midas:am:` ������ `SET`. Commit `df15a01`. |
| 2026-05-14 12:28 | **Phase 2.10 � Fix 2: from-context � delete flow parser.** ��������: ��� ������� ��������� ������� > ��������� > ������� > ��������� � ������ ������� ������� �������� ������ �������������� success card. ������: `parseTxCallback` �� ����� `parts[4]` ��� `tx:d:ask` � `tx:d:yes` � �������� `from='s'` ������� ��� ��������. Fix: `transaction-keyboard.service.ts` � `const from = parts[4]`; return � `from` ��� ����� action. ������ `tx:view` ��������� ����� `from==='s'` � ������ `closeCallback = tx:done:{txId}`. Commit `8894b92`. |
| 2026-05-14 12:37 | **Phase 2.10 � Fix 3: Double-lock sentinel key.** ��������: ���� ����� Fix 1 success card ������ ��������� (race condition ����� background-workers � telegram-bot, ��� ���������� ������). ������� � ������� ����������: (1) `notifications.worker.ts` ��� `isSuccessCard`: SET `midas:success_card:{sentMessageId}` = '1' (TTL 30 ����), ����� DEL `midas:am:`. (2) `webhook.route.ts` step-7: ����� `deleteMessage(amId)` ��������� `EXISTS midas:success_card:{amId}` � ���� sentinel ����, ��������� �� ��������� (������ ��������� pointer). ��� ����� �������� ����������. tsc 0 ������ ��� ����������. Commit `b869c03`. |
| 2026-05-14 17:30 | **Phase 2.10+ Gate Fix � Frozen UI ��� ������������ ����� ����������.** ��������: TX1 ��������� ����� ����� > TX2 (webhook step-7) ������� ����� (gate_sent ��� �� ����������) > ai-parse gate ��������� ����� �������� � ������� � ������������� gate_sent > TX3 (webhook step-7) ������� gate-�������� (gate_sent �� ����������!) > ai-parse ������ (gate_sent SET > silently ignore) > TX4, TX5... ����: ��������� ��������, ���������, ������ ��� � **�������**. **Fix 1 (webhook.route.ts ������ 5446�5458):** `const gateSentActive = await redisConnection.exists('midas:gate_sent:...')`. ���� ������� � `deleteMessage` � `clearActiveMessageId` �� ����������. Gate-�������� ������� ������� ��� TX3, TX4... **Fix 2 (webhook.route.ts ������ 1539, ia:pk: handler):** `redisConnection.del('midas:gate_sent:...')` ����� `setDraftAccountId` � ���������� flow ����������������� ����� ����� ������ �����. **Fix 3 (ai-parse.worker.ts):** Gate �������������� ������ ����� ������ (inline keyboard � �������� ������ + ?? ������) ����� `pendingDraft.accountId === null` � ������ ������ confirm-����������. **��������� ���� gate_sent:** SET ai-parse.worker (��� gate) > DEL ia:cancel (������ 1432, �� �����) / ia:pk: (���������) / approve/reject confirmation.worker (������ 268, �� �����) / TTL auto 1h. Scope: 2 ����� (webhook.route.ts, ai-parse.worker.ts) + ������� fix-stuck-draft.mjs. tsc 0 ������. git commit `8d25ec1`, push origin main ?. Railway: Midas ? Online, background-workers ? Online. |
| 2026-05-14 20:00 | **Phase 2.5+ � Currency-Aware Picker: Bot Layer (telegram-bot).** ��������: � ������ ������ ��� USD-���������� ����������� USDT-����, ���� ��� ���������� � �� �� �������������� � ����. **���������� (4 �����):** (1) `account-currency-validator.service.ts` � ��������� ������� `isKnownCurrency(code)`: ��������� ��� �� ��� ���������� (FIAT_SET + STABLECOINS + CRYPTO_SET). ������������� �������� ��������� ����� ���� �UDS� ��� ���л. (2) `clarification.service.ts` � � `validateCurrencyCode()` ��������� ������ �������� `!isKnownCurrency(upper)` > ������� `null` �� ������ � ��. (3) `account.service.ts` � `getWorkspaceAccountsWithBalances()` �������� ������������ 4-� �������� `parsedCurrency?`. ����� SQL-�������: ���� tx � ���� > exact-match ������� + ��������� �������; ���� ����������/������ > ������ exact match. (4) `account-inline-keyboard.service.ts` � `getPickerScreenText(intent, parsedCurrency?)` ��������� ����������� ���������; `getPickerEmptyText(parsedCurrency?)` � ���� USDT-������ ������ ������ ���������. `webhook.route.ts` � ������������ `draft.parsed_currency` � 3 entry points (sendAndStorePreview, ia:delink, ia:showpicker). ������ ������ ���� � TS6133 (ACCOUNT_PICKER_EMPTY_TEXT � ������� �� �� ������������). ���������� �������� `04f7e81`. |
| 2026-05-14 20:10 | **Phase 2.5+ � Currency-Aware Picker: Worker Layer (background-workers). Root Cause Fix.** ����������: ��������� ����� �������� ��������� � `ai-parse.worker.ts` (background-workers), � �� � `telegram-bot`. ��������� � `account.service.ts` (telegram-bot) �� initial picker �� ������ �����. **���������� (`ai-parse.worker.ts`):** ��������� ��������� ��������������: `PICKER_STABLECOINS` (10 �������), `PICKER_KNOWN_CRYPTOS` (27 �������), `classifyPickerCcy(code)`, `filterPickerAccounts(accounts, txCurrency)` � ������ ������ `account.service.ts`. ��������� � 2 ������: (A) **Initial picker** (������ ~620) � ��������� �� `aiData?.currency` (����� AI ������ currency, �������� �USDT�); (B) **Gate picker** (������ ~340) � ��������� �� `pendingDraft.parsedCurrency` (�������������� ������ ��� gate-����������). ���� ����������: `{USD tx}` > [USD-�����] + [������ �������]; `{USDT tx}` > [������ USDT-�����]. tsc 0 ������ (��� ����������). git commit `0085d8f`, push origin main ?. Railway auto-deploy triggered. |
| 2026-05-15 02:00 | **Balance Phase A � Grouped UI ���������.** `balance-keyboard.service.ts` (MODIFY): `GroupType` union, `GROUP_EMOJI` map, `GROUP_ORDER` priority, `classifyAccountGroup(name, currency)` ��������� (�����/�����������/������-��������/��������/������), `buildBalanceListKeyboard` � ������������ � emoji-����������, `export formatBalanceShort`. `balance.service.ts` (MODIFY): ���������������� ����� getBalanceData() � ������ �����, ����� CURRENCY_TOTALS_SQL. tsc 0 ������. Commit `4a1748c` push to main. Railway auto-deploy ?. |
| 2026-05-15 02:27 | **Balance Phase B-1 � DB Migration ���������.** `packages/database/migrations/1779800000000_account-parent-and-subtype.js` (NEW): `parent_account_id VARCHAR(26) FK ON DELETE CASCADE`, `sub_type TEXT NOT NULL DEFAULT 'general'` CHECK constraint, `idx_account_sources_parent` (partial). ������ ESM-�������� `1779400000000` (exports > export const). �������� ��������� `node-pg-migrate up --check-order false`. �����: FK 31/31 ?, ������� initial_balance+income?expense ?, INSERT ���������� �� �������� ?. Commit `75156b9`. |
| 2026-05-15 02:30 | **�������� workflow_state.md ��� Phase B-2 handoff.** Section 1 (status), Section 2 (����), Section 8 (�����), Section 9 (������), Section 10 (�������). ��������� ���: Phase B-2 (PER_ACCOUNT_SQL + ������� +/L + ��������� ��������). |
| 2026-05-15 23:40 | **Balance Phase B-2 � Hierarchical Ladder View ���������.** `balance.service.ts`: `PER_ACCOUNT_SQL` �������� `a.parent_account_id`; `AccountBalanceRow` + `parent_account_id: string | null`; `getBalanceData()` ������ childrenMap/childCountMap, �������� +/L ������� ��� parent>children; �������� ����� ��� ��������� (backward compat). `balance-keyboard.service.ts`: `BalanceAccountRow` + `parentAccountId?`+`childCount?`; `BalanceCallbackCmd` + `add_currency`; `parseBalanceCallback` `bl:ac:{id}` � SEC-01 compliant; `pluralizeCurrency()` (������/������/�����, mod10/mod100); `buildBalanceListKeyboard()` ��������� � �������� parents/children, parent � ������: aggregation button + indented `L CURRENCY � balance` child rows + `? �������� ������` (bl:ac:{parentId}); �������� ����� � Phase A/LD++ rendering. tsc 0 errors. Commit `d04bcba` pushed to main. Railway auto-deploy triggered. |
| 2026-05-14 22:00 | **Hotfix: ������ "?? ������" � ������ ������ + "����" ����� USDT + ������-�������.** (1) `account-inline-keyboard.service.ts` (MODIFY) ������ 381�383: ������ `buildAccountPickerV2Keyboard` �?? ������ �������� � `ia:pk:back:{draftId}` > `ia:cancel:{draftId}`. �� �����: ������� ������� ���������� � �������� ������ � �������� [?? ��������|?? ������]+[?? ������� ����]. ����� �����: `ia:cancel` handler ����������� ��������� > �? �������� ��� ������, ������ ��������� ������ `rejected`, ������ Redis. (2) `packages/ai-core/src/prompts.ts` � �������� `"����"` � ������ ������� USDT (������ 37): ���� `"����", "�����", "tether", "usdt"` > ����� `"����", "����", "�����", "tether", "usdt"`. (3) `packages/ai-core/src/prompts.ts` � ��������� 2 ������� � ������ `-- Partial (amount missing) --`: `"����� �������� ����"` > `{intent:expense,currency:USDT,item_hint:��������,confidence:0.75}` � `"����� �������� usdt"` > `{intent:expense,currency:USDT,item_hint:������������,confidence:0.75}`. ����: Claude ������ ���������� `item_hint` ���� ����� ��� `amount`. tsc 0 ������. git commit `ccaec87`, push origin main ?. Railway auto-deploy triggered. |
| 2026-05-14 20:00 | **Phase 2.5+ � Currency-Aware Picker: Bot Layer (telegram-bot).** ��������: � ������ ������ ��� USD-���������� ����������� USDT-����, ���� ��� ���������� � �� �� �������������� � ����. **���������� (4 �����):** (1) `account-currency-validator.service.ts` � ��������� ������� `isKnownCurrency(code)`: ��������� ��� �� ��� ���������� (FIAT_SET + STABLECOINS + CRYPTO_SET). ������������� �������� ��������� ����� ���� �UDS� ��� ���л. (2) `clarification.service.ts` � � `validateCurrencyCode()` ��������� ������ �������� `!isKnownCurrency(upper)` > ������� `null` �� ������ � ��. (3) `account.service.ts` � `getWorkspaceAccountsWithBalances()` �������� ������������ 4-� �������� `parsedCurrency?`. ����� SQL-�������: ���� tx � ���� > exact-match ������� + ��������� �������; ���� ����������/������ > ������ exact match. (4) `account-inline-keyboard.service.ts` � `getPickerScreenText(intent, parsedCurrency?)` ��������� ����������� ���������; `getPickerEmptyText(parsedCurrency?)` � ���� USDT-������ ������ ������ ���������. `webhook.route.ts` � ������������ `draft.parsed_currency` � 3 entry points (sendAndStorePreview, ia:delink, ia:showpicker). ������ ������ ���� � TS6133 (ACCOUNT_PICKER_EMPTY_TEXT � ������� �� �� ������������). ���������� �������� `04f7e81`. |
| 2026-05-14 20:10 | **Phase 2.5+ � Currency-Aware Picker: Worker Layer (background-workers). Root Cause Fix.** ����������: ��������� ����� �������� ��������� � `ai-parse.worker.ts` (background-workers), � �� � `telegram-bot`. ��������� � `account.service.ts` (telegram-bot) �� initial picker �� ������ �����. **���������� (`ai-parse.worker.ts`):** ��������� ��������� ��������������: `PICKER_STABLECOINS` (10 �������), `PICKER_KNOWN_CRYPTOS` (27 �������), `classifyPickerCcy(code)`, `filterPickerAccounts(accounts, txCurrency)` � ������ ������ `account.service.ts`. ��������� � 2 ������: (A) **Initial picker** (������ ~620) � ��������� �� `aiData?.currency` (����� AI ������ currency, �������� �USDT�); (B) **Gate picker** (������ ~340) � ��������� �� `pendingDraft.parsedCurrency` (�������������� ������ ��� gate-����������). ���� ����������: `{USD tx}` > [USD-�����] + [������ �������]; `{USDT tx}` > [������ USDT-�����]. tsc 0 ������ (��� ����������). git commit `0085d8f`, push origin main ?. Railway auto-deploy triggered. |
...
| 2026-05-15 18:45 | **fix(draft): Back button in currency picker → confirm card (commit 45b3ac7). DEPLOYED.** `webhook.route.ts` line 2995: `draft:edit:{draftId}` → `draft:back:{draftId}` on the "◀️ Назад" button inside the currency picker screen. Result: pressing Back from currency selection (Screen 2) now restores the full confirmation card (Screen 3) directly, skipping the "Что изменить?" submenu (Screen 1). 1-line change. tsc 0 errors. git push → Railway auto-deploy triggered. |
| 2026-05-15 19:15 | **feat(ai): Phase 2.7 — spoken-number normalization, stop-words, Ukrainian verbs, clarification context (commit 4a38176). DEPLOYED.** `ai-parse.worker.ts`: (1) Deterministic spoken-number normalization: "сто" → "100", "двести пятьдесят" → "250" via regex before Claude. (2) Stop-word guard: "только", "просто" etc. won't trigger false person/account names. (3) Ukrainian verb normalization: "заплатив" → "заплатил". (4) Clarification context injection: second-turn AI call now includes original draft's category_hint and item_hint so Claude maintains intent across turns. Railway auto-deploy. |
| 2026-05-15 19:10 | **fix(ux): Delete both dead card and voice status msg on new preview (commit 3696042). DEPLOYED.** `notifications.worker.ts`: When a new preview arrives, now cleans up BOTH the old «Отменено» dead card AND the voice processing status message. Prevents UI clutter on rapid voice input. Railway auto-deploy. |
| 2026-05-15 19:05 | **fix(ux): Auto-delete stale «Отменено» card on next user message (commit fb5f140). DEPLOYED.** `webhook.route.ts`: Dead-card cleanup fires on any new user message. `midas:dead_card:{chatId}` checked and deleted in the main text interceptor. Railway auto-deploy. |
| 2026-05-15 19:00 | **fix(stt): Normalize crypto tickers before AI parse (commit f4eb9c1). DEPLOYED.** `ai-parse.worker.ts`: Pre-processing step normalizes spoken/mistyped crypto tickers ("юздт" → "USDT", "ефир" → "ETH") before Claude. Eliminates USD vs USDT confusion from voice input. Railway auto-deploy. |
| 2026-05-15 20:00 | **feat(draft): 2-level AI-first category picker — group browse + emoji (commit f87393a). DEPLOYED.** `webhook.route.ts`: Replaced flat category list with 2-level hierarchical picker. Screen 1: AI-hint row (✨ CategoryName) + two group buttons [🛒 Жизнь] [💼 Бизнес]. Screen 2: per-group flat list with emoji icons per category (28 categories, emoji map). Callback: `clar:cat:{catId}:{draftId}`. Railway auto-deploy. |
| 2026-05-15 20:10 | **fix(draft/cat): Remove без-категории, smart flat-list, header emoji fix (commit 17cc9a9). DEPLOYED.** `webhook.route.ts`: (1) Removed «Без категории» option — every transaction must have a category. (2) Smart auto-flatten: if workspace has ≤6 categories total, skip group screen and show flat list. (3) Fixed doubled group header icon. Added emoji for Разное/Питомцы/Дом. Railway auto-deploy. |
| 2026-05-15 20:15 | **db: Merge legacy «Разное» category into canonical «Другое» across all workspaces (commit 60cb4d9). DEPLOYED.** Migration `1779900000000_merge-raznoye-into-drugoye.js`: `UPDATE transaction_drafts SET category_id` + `UPDATE transactions SET category_id` to point to Другое, then `DELETE FROM categories WHERE name='Разное'`. Eliminates naming duplicate. Railway auto-deploy. |
| 2026-05-15 20:20 | **fix(cat): Dedup AI hint from flat-list; backfill 28 default categories for all workspaces (commit 63cd4a3 + 6ad62d5). DEPLOYED.** Migration `1780000000000_backfill-28-default-categories.js`: Inserts 28 canonical categories (Еда, Транспорт, Одежда, …) for every workspace that doesn't already have them. `webhook.route.ts`: AI-hint button excluded from the flat list to avoid duplicate entries. Railway auto-deploy. |
| 2026-05-15 20:30 | **fix(cat): Show selected category in preview card; remove double emoji from group header (commit 748a81c). DEPLOYED.** `clarification.service.ts` (`getDraftFields`): Added `LEFT JOIN categories c ON td.category_id = c.id` → returns `c.name AS category_name`. `webhook.route.ts` (`confirmPreviewFull`): Uses `category_name ?? parsed_category_hint` so confirmed card shows the user-selected category, not the stale AI hint. Also removed redundant `📁` prefix from Screen 2 group header. tsc 0 errors. Railway auto-deploy. |
| 2026-05-16 13:00 | **Phase 2.6 — State-Aware Reminders + Chat Hygiene + item_name UX (DEPLOYED).** **[1] DB Migration `1780200000000_draft-current-screen.js`:** Added `current_screen TEXT` column (`screen1` / `screen1b` / `screen2`) to `transaction_drafts`. Updated `system_find_drafts_needing_reminder` SQL function to also return `account_currency` (needed for cross-currency button labels). **[2] `draft-expiration.service.ts` (background-workers):** Added `accountCurrency` to `ReminderDraft` interface and to the mapper that reads SQL results. **[3] `draft-expiration.worker.ts` (background-workers):** Worker now branches on `draft.currentScreen` (`screen1` / `screen1b` / `screen2`) to render state-matching reminder cards (correct buttons for each UI state). Button labels for `screen1b`/`screen2` now use `draft.accountCurrency` (account's real currency) instead of `parsedCurrency` (transaction currency) — fixes cross-currency label bug. **[4] `webhook.route.ts` (telegram-bot):** `updateDraftCurrentScreen()` called at 3 transition points: `ia:pk` (→ `screen1b`), `ia:delink` (→ `screen1`), cross-currency amount submission (→ `screen2`). **[5] `notifications.worker.ts` (background-workers):** Automatic deletion of previous preview card using `deleteMessageId` payload field — stale cards are cleaned up before new reminder is sent. **[6] `screen-builder.ts` (telegram-bot):** `buildPreviewScreen` refactored — `item_name` is now rendered as a **separate paragraph** above the amount blockquote instead of being embedded inside it. Prevents visual clutter when users input extended/nonsense text. Also applied Prettier auto-format (alignment whitespace removal — no logic change). Commits: `cc26652` (core Phase 2.6) → `7ea5bf2` (style: screen-builder format). Railway auto-deploy ✅. |
...
- project_config.md
- workflow_state.md
- docs/product-roadmap.md (����������� ���� �������� �������� � Phase 1.23�2.5)
...
- `workflow_state.md`
- `project_config.md`
- `docs/product-roadmap.md` < **�������� ������** ��� ��������� ��� (1.23�2.5)
...
**��������** � ��������� ����� ������ ���:
- ���������� ���� ��� ������� �������
- ����������� review gate (Traceability / Security / Scope Guard)
- ��������� MCP ������������
- git checkpoint
- context reset / handoff � ����� ���
- ����� ������� high-risk ���� (DB, security, payments, auth, deploy)
...
1. ������������ ����� `/start` � ��� ����.
2. `webhook.route.ts` > `resolveWorkspace()` > �������� `system_find_or_create_user()` (SECURITY DEFINER, atomic, pg_advisory_xact_lock).
3. ��������: **workspace** (default_currency=USDT, timezone=UTC), **workspace_membership**, **default account_source** (��� ����������, USDT), **default category** (������).
4. ��� ���������� �������������� ��������� � ReplyKeyboard (`is_persistent: false`, `resize_keyboard: true`):
   ```
   ������ 1: [?? ������]  [?? �����]
   ������ 2: [?? ����������]  [?? ���������]
   ```
5. ���� � ������������ **0 ������** > ��� ����� ���������� guided onboarding keyboard (`buildStartOnboardKeyboard`).
6. Greeting-��������� **������� �� ���������** � ��� �������� ReplyKeyboard.
...
2. ��� ���������� ������ ����� �������� � blockquote-���������:
   ```
   ������� �������� �����:
   <blockquote>��������: �������� � �������� � ����� � Monobank</blockquote>
   ```
3. ������������ ������ ����� > `name_input` text interceptor.
...
- ��� ���������� no-match �����:
  ...
  ```
- ������:
  - `[? ������� ����]` (ac:cus:save) > ��������� ��� `pendingName`, `isCustomName=true`, ��������� � `cur_pick`.
  - `[?? �������� ��������]` (ac:cus:keep) > ������� � `name_input`.
  - `[?? � ���� �����]` (ac:type:back) > ������� �� ��������� �����.

#### 1.3 �������� ��������� (ac:type:cash)

- �������� ����������� �������������: ��������� {CURRENCY}� (��� ����� �������� ����� ������ ������).
- ������������ ����� ����� currency picker (��� `cur_pick`).
- **��� ������ ����� ��������.**

#### 1.4 �������� �������-����� / �������-������

- **������-����� (ac:type:exchange):** ���������� paginated picker ���� (5 ��������: Binance/Bybit/OKX/Kraken/Huobi + ?? ����).
- **������-������ (ac:type:wallet):** ���������� sub-picker: crypto / e-wallet / TON / Lightning.
  - Lightning > ������������� ������ BTC, ������ currency picker.
  - ��������� > ���� � crypto currency picker.
- Paginated pickers ������/���� � ��������� `[??][N / Total][??]`, ������ ��� ������� (noop �� �����).
...
2. ��� ���������� ������:
   ...
   ������� ��� ��� ��������: RUB, ������, bitcoin...
   ```
3. ������������ ������ ����� > `cur_search` text interceptor.
4. `searchCurrencies(query, pool)` � fuzzy + �������������� (rub/��� > RUB, dollar/������ > USD, btc > BTC).
5. **�������:** ���������� ������ ����������� + `[?? ��������� � ������]` (ac:cur:list).
6. **�� �������:** ������ ������ ���. ����������: USD, RUB, BTC...�.

**����� ������ (ac:cur:{CODE}):**
- ���� �������� � ��: `addAccountWithCurrency(workspaceId, userId, name, currency)` > INSERT � `account_sources`, ��� `manual`.
- FSM ��������� � ��� `bal_input`.
...
- **���� �����** > text interceptor `bal_input` > `setAccountBalanceById()` > `initial_balance` � ��.
- **`[? ����������]`** (ac:bal:s) > ������ ������� 0.
...
������������ �������� ����� ��� � `buildFinishOnboardKeyboard` > ��������� ����� 1�3.
...
���� �������� (������ ����):
- ������ `[?? ��������]` > ����� currency picker (��� name_input) > ������� `[???? RUB]` > ������ ������ `5000` > success screen.

����� � ����� `buildFinishOnboardKeyboard`. ������������ �������� `[? ���������]` (ac:fin):
- Redis-���� `midas:ac:` ���������.
- ��������� ��������� (`deleteMessage`).
- ������������ `sendMessageWithReplyKeyboard` � ReplyKeyboard ���������� �����.
- ���� ������ �� ������-�������� (`bl:source` � Redis) > ������� � ������. ����� � ��������� ����� ��� ������!�.
...
**�������:**
1. `webhook.route.ts` � ��������� �������� ��� text interceptors (��� �������� Redis-������).
2. �������� � ������ AI parse > `addJobToWebhookIngestionQueue()`.
...
2. System prompt: MULTILINGUAL RECOGNITION (RU/EN/UA) + FUZZY MATCHING + 30-������������ ���������� + 500+ ������� ���� + DISAMBIGUATION RULES.
3. **���������:**
   ```json
   { "intent": "expense", "amount": "150", "currency": "RUB", "category_hint": "���� � ���������", "confidence": 0.95 }
   ```
4. Post-processing (safety net): 7 ����� regex, negation guard, confidence boost.
5. `ALLOWED_CATEGORIES` ���������: ���� `category_hint` ? set > ���������� �� �������.
6. `CategoryResolverService`: exact DB match > 200+ alias map > fallback.
7. **Dead card cleanup:** ���� � Redis ���� `midas:dead_card:{chatId}` (������ ? ��������) > `deleteMessage` ����� ��������� preview.

#### 5.3 �������� ��������� � preview

1. `createDraft()` > INSERT � `transaction_drafts` (������ `pending_user`).
2. `notifications.worker` > ���������� preview-�������� � ���:
   ```
   ? ���� � ���������
   ������ � 150 ?
   [? ��������]  
   [?? ��������] [?? ������]
   ```
3. `midas:preview:{draftId}` (TTL 600s) > ��������� message_id ��������.
...
3. INSERT � `transactions` (intent=expense, category=���� � ���������, base_amount=150, currency=RUB, account_id=��������, base_currency=RUB).
4. `confirmation.worker` ������ `midas:preview:{draftId}` > `editMessageText` > preview ������������ � confirmed card:
   ```
   ? ��������!
   ? ���� � ���������
   ������ � 150 ? � ��������
   [?? �������� ������]
   ```
5. `midas:preview:{draftId}` ��������� �� Redis.

**������������ �������� `[?? ������]`:**
- `draft_status` > `rejected`.
- preview-�������� ������������� > �? ��������.
- ����������� � `midas:dead_card:{chatId}` (TTL 24h) � ������������ ��� ��������� preview.

#### 5.5 ���� Claude �� ��������� ������ (awaiting_cur)

- `midas:awaiting_cur:{chatId}` (TTL 600s) �������� ���� ���� ����� �� ��� ������ � ��� `midas:cur_set:{workspaceId}`.
- ��������� ����� ������������ ��������������� ��� ������: ����� > EUR, �150 ��� > RUB.
...
- `needs_clarification` ������ ���������.
- ������������ ������� ������: ������ �����?�.
- `midas:clar:{userId}:{chatId}` (TTL 300s) > ��������� ����� � �����.
...
| ���� | TTL | ���������� |
|---|---|---|
| `midas:ac:{userId}:{chatId}` | 300s | State ������ ���������� (step, name, currency, pendingName, isCustomName, cur_search) |
| `bl:source:{userId}:{chatId}` | 300s | ����: ��������� ����������� �� ������-�������� |
| `midas:preview:{draftId}` | 600s | message_id preview-�������� |
| `midas:dead_card:{chatId}` | 24h | message_id �������� ? ��� ������������ |
| `midas:awaiting_cur:{chatId}` | 600s | �������� ����� ������ |
| `midas:clar:{userId}:{chatId}` | 300s | �������� ����� ����� ��� clarification |
| `midas:cur_set:{workspaceId}` | - | ���� ������������� ������ (�� ����������� ��������) |
...
> ���� ������ � ����� ��������. ����������� ��� ���������� ������ ����.
> ��������� ����������: 2026-05-15 14:20 (UTC+3)

### ✅ Phase 9 — Auto-Pilot Primary Account (Transaction Routing)

**Статус: ✅ DEPLOYED** (commits 378b976, 1bd5ae8 · Railway SUCCESS · 2026-05-15)

**Суть:** Если у пользователя установлен Основной счёт (⭐ в Balance > Счёт), бот автоматически применяет его валюту и account_id к черновику — без экрана «В какой валюте?» — сразу показывает карточку подтверждения.

**Поток (новый):**
`
Пользователь: «купил авто 1000»
AI: parsed OK (amount=1000, intent=expense, currency=null)
  └╴ ai-parse.worker: aiCurrency=null
  └╴ getWorkspaceAccountsForPicker() → найден primary (is_expense_default=true)
  └╴ setDraftAccountId() + UPDATE parsed_currency via withTenantTransaction
  └╴ buildPreviewScreen() + buildConfirmKeyboard() → notificationsQueue
  └╴ return  ← пикер валюты НИКОГДА не показывается
`

**Ключевые решения:**
- is_expense_default — **не колонка** в ccount_sources, вычисляется как (a.id = w.default_expense_account_id) через JOIN с workspaces
- Основной счёт хранится в workspaces.default_expense_account_id (VARCHAR FK)
- getWorkspaceAccountsForPicker использует pool.query без RLS — это правильный паттерн для background-workers
- Приоритет: ⭐ Primary Account → Redis cur_set → «В какой валюте?»

**Изменённые файлы:**

| Файл | Что сделано |
|---|---|
| pps/background-workers/src/services/draft.service.ts | WorkspaceAccountEntry + поле is_expense_default. SQL: JOIN workspaces w, (a.id = w.default_expense_account_id) AS is_expense_default, GROUP BY + w.default_expense_account_id |
| pps/background-workers/src/workers/ai-parse.worker.ts | Блок if (!aiCurrency) переписан: getWorkspaceAccountsForPicker().find(), setDraftAccountId(), withTenantTransaction + confirm-карточка, 
eturn. Добавлен импорт withTenantTransaction |
| pps/telegram-bot/src/services/balance.service.ts | getDefaultAccount() (вспомогательная функция, резервный webhook-путь) |
| pps/telegram-bot/src/routes/webhook.route.ts | getDefaultAccount в clarification-flow (резервный путь) |

---

### ✅ Phase B — Hierarchical Account Balance UI (N26/Revolut Style)

**Статус: ✅ DEPLOYED** (commits cb37de6, c4ba46c, 0b6530, 21e0a6f · Railway SUCCESS · 2026-05-15)

**Суть:** Полный редизайн раздела Баланс под профессиональный стиль N26/Revolut. Иерархическая структура счетов (parent/child), роли счетов, компактный однострочный формат.

**Изменённые файлы:**

| Файл | Что сделано |
|---|---|
| pps/telegram-bot/src/services/balance.service.ts | Компактный формат: 1 строка = 1 счёт. CCY_SYMBOL (17 валют). 
oleSuffix: роль добавляется суффиксом (⭐ основной). GROUP_LABEL → Title Case. ACCOUNT_DETAIL_SQL + parent_account_id |
| pps/telegram-bot/src/services/balance-keyboard.service.ts | GROUP_EMOJI. uildAccountActionsKeyboard(…, canAddCurrency) — параметр только для top-level (parent_account_id === null). Роль ⭐ — только внутри дочернего счёта |
| pps/telegram-bot/src/routes/webhook.route.ts | 7 вызовов uildAccountActionsKeyboard: условие parent_account_id === null |

**DB:** parent_account_id (FK → ccount_sources.id) + sub_type (TEXT) в ccount_sources.
...
| ��� | ��� ������� | ������ |
|---|---|---|
| ��� 1 | `item-category-detector.service.ts` � ����-����������� ��������� �� �������� ������/������ (200+ �������, 9 ���������, Maybach>���������) | ? |
| ��� 2 | `account-currency-validator.service.ts` � ���������� ������������� ��� ����+������ (����+USDT = ?, �����+USDT = ?) | ? |
| ��� 3 | `anomalyBadge()` � ������� � ���������� `??` ��� �������������� ������������ ������ | ? |
| ��� 4 | `ai-parse.worker.ts` � ���� �Active Draft Gate�: ����� Account/XFX-��������� UI ����������� ��� �������� ��������� | ? |

### ? ��������� � Phase 2.5+ (Currency-Aware Account Picker)

> **��������:** USDT-���� ����������� � ������ ��� USD-����������. ������� � ��������� ����� �������� � `background-workers`, � �� � `telegram-bot`, ������� ��������� � `account.service.ts` (telegram-bot) �� ���� �� ������.

| ��� | ���� | ��� ������� | ������ |
|---|---|---|---|
| 1 | `account-currency-validator.service.ts` | `isKnownCurrency()` � ��������-������ �� ��������� ����� (UDS, ���) | ? |
| 2 | `clarification.service.ts` | `validateCurrencyCode()` > ������ �������� `isKnownCurrency()` ����� ������� � �� | ? |
| 3 | `account.service.ts` | `getWorkspaceAccountsWithBalances(parsedCurrency?)` � ������: ����>������� ���, ����������/������>exact only | ? |
| 4 | `account-inline-keyboard.service.ts` | ����������� ��������� � `getPickerEmptyText(parsedCurrency?)` | ? |
| 5 | `webhook.route.ts` | ������������ `parsed_currency` � 3 entry points (preview, delink, showpicker) | ? |
| 6 ? | `ai-parse.worker.ts` | **Root-cause fix:** `filterPickerAccounts()` + `classifyPickerCcy()` ��������� � initial picker (`aiData.currency`) � gate picker (`pendingDraft.parsedCurrency`) | ? |

**������������� ����:** � Midas ��� ����������� ��������� ������. ����� ��������� ������ ������ ������� ���������� ����� ����������:
- `apps/telegram-bot` � ������ ��������� (ia:delink, ia:showpicker)
- `apps/background-workers` � ��������� ����� ����� AI parse

---

### ?? Phase 3.0 � DB Schema: ������ ������������� ��������� (�����������)

> **���������: �������.** ������� ��������� (��� 2) � �������������, �������� �� `AccountOnboardState` �� Redis.
> ���� Redis-���� ���� ��� ������������ ������ ���� ������������� ���� � ��� ����� ����������.
> Phase 3.0 ��������� ������� �� **100% �������, �����-enforced ���������**.
...
**���������� ��� �������� �����:**
- � `account.service.ts` > `addAccountReturningId()` � `addAccountWithCurrency()`:
  ��������� `accountType`, `walletSubtype`, `providerKey` �� `AccountOnboardState` � ���������� � ��.
- � `webhook.route.ts` > `cmd=currency` handler: ���������� `state.accountType`, `state.walletSubtype`, `state.name.toLowerCase()` ��� `providerKey`.

**������������� ��� �����������:**
- `buildAccountPickerForDraft` � `buildAccountPickerV2Keyboard`:
  ������ ��������� �� ����� > ������ `account_type` �� ��, ���������� � `validateAccountCurrency()`.
  ��� ������ `??` badge �� 100% ������.

**������������� ���������� (�����������):**
- ���������� ������� `account_type` �� ������������ �������� ������ ����� ���� � `BANK_PRESETS`/`EWALLET_PRESETS`/`EXCHANGE_PRESETS`.
- ��� ��� �� ������� > `account_type = 'custom'`.
...
| `apps/telegram-bot/src/services/account.service.ts` | MODIFY � ��������� ��������� addAccount* |
| `apps/telegram-bot/src/routes/webhook.route.ts` | MODIFY � ���������� ��� � addAccount* |
| `apps/telegram-bot/src/services/account-inline-keyboard.service.ts` | MODIFY � ������ ��� �� �� ������ ��������� |
| `apps/telegram-bot/src/services/account-currency-validator.service.ts` | MODIFY � ������ ���������-���� �� ��������� (������ �� ��) |
...
- �������� ���������: `�����������`, `�������`, `�������`, `����������`
- �������� 150+ ��������� �������: ���, ѳ����, Kaspi, OLX, Wildberries, Ozon, ����
- �������� ��������������: �starbaks� > Starbucks, �mak� > McDonald's
...
| ���� | �������� | ��������� | ������ | ������� |
|---|---|---|---|| 2026-05-15 09:20 | **Balance UI Polish B-9+ � compact text layout + Add Currency fix.** alance.service.ts: (1) Compact text � ������ ������ ������ ����� ������� ������ ������ � ����� ���������� � ������ ������; GROUP_LABEL > Title Case (�� ALL CAPS); ����-������ ��������� �� ������. (2) ACCOUNT_DETAIL_SQL + GROUP BY ��������� � �������� .parent_account_id; AccountDetailRow/AccountDetailData + parent_account_id: string | null. alance-keyboard.service.ts: uildAccountActionsKeyboard � �������� ������������ hasChildren > canAddCurrency, ������ ��������� ������ ������������ ��� ���� top-level ������ (parent_account_id === null), �� ������ ������� �����. webhook.route.ts: 7 ������� uildAccountActionsKeyboard � ������� detail.child_count > 0 �������� �� detail.parent_account_id === null. tsc 0 ������. Commits cb37de6. Railway auto-deploy. |
| 2026-05-15 13:10 | **Phase 9 v4 - Auto-Pilot Primary Account (DEPLOYED).** draft.service.ts (background-workers): WorkspaceAccountEntry + is_expense_default (computed via JOIN workspaces), SQL переписан с .is_expense_default → (a.id = w.default_expense_account_id) AS is_expense_default, GROUP BY дополнен. i-parse.worker.ts: блок if (!aiCurrency) полностью переписан — Primary Account check через getWorkspaceAccountsForPicker().find(a.is_expense_default), патч parsed_currency + ccount_id через withTenantTransaction/setDraftAccountId, confirm-карточка собирается здесь же + 
eturn (пикер валюты не показывается). Добавлен withTenantTransaction в импорты. Исправлена корневая причина 3 итераций отладки: pool.query напрямую к ccount_sources с is_expense_default=true всегда возвращал 0 строк — такой колонки нет, она вычисляется через JOIN с workspaces. tsc 0 ошибок. Commits 378b976, 1bd5ae8. Railway SUCCESS. || 2026-05-15 09:30 | **Balance UI Polish � N26/Revolut professional redesign.** alance.service.ts: (1) ����� ������ � 1 ���� = 1 ������ ������-���� (? ��������) � 22 010 213 ?� ������ 2-��������� �L balance currency�. (2) ������� CCY_SYMBOL � ������� ����� ? $ � ? ? ? � (17 �����) ������ �����. (3) 
oleSuffix � ���� ��������� <i>(? ��������)</i> ����� ����� (Variant A). (4) ��������� ?? ������ > ?? ������ (��������� � reply-keyboard). alance-keyboard.service.ts: (1) GROUP_EMOJI ��������� � ?? �����, ?? ��������, ?? ������. (2) GROUP_LABEL > Title Case. (3) ������ � CCY_SYM ������� ��������; ����-������� ' ?' (icon-only, ��� ������ � ������). tsc 0 ������. Commits c4ba46c, 0b6530, 21e0a6f. Railway auto-deploy. |

---|
| **3.0** | DB Schema: account_type/wallet_subtype | ?? ������� | ? ��������� | Phase 2.5 ? |
| **3.1** | ���������� ������� ��������� | ?? ������� | ?? ������������� | Phase 3.0 |
| **3.2** | ����� 3.0: ������������ ��������� | ?? ������� | ?? ������������� | Phase 3.0 |
