...
> **Обновлён:** 2026-05-23 13:35 (UTC+3)
...
| Параметр | Значение |
|---|---|
| **PHASE** | Phase 5.1 — Voice-Activated Quick Edits ✅ DEPLOYED |
| **STEP** | Сессия 2026-05-23 09:00–13:35. Phase 5.0 + 5.1: Context-Aware Quick Edits (текст + голос). Все 4 sub-phase завершены и задеплоены. |
| **AGENT STATUS** | turbo 5/5 successful, 0 TypeScript errors. Commit `174b8f7` pushed to origin/main. |
| **DEPLOYMENT** | Railway (spirited-happiness) — auto-deploy из GitHub main. Последний commit на сервере: `174b8f7`. Health: https://midas-production-f4f1.up.railway.app/health > ok |
| **DB STATE** | `transfer_group_id`: TEXT ✅ \| `current_screen`: TEXT ✅ \| `account_type`: TEXT nullable ✅ \| `provider_key`: TEXT nullable ✅. Новых миграций в Phase 5.0/5.1 нет. |
| **DATABASE_URL (public)** | `postgresql://postgres:PLLSqArtPUoQsAYmvrpsmavfQMewgTRh@hopper.proxy.rlwy.net:46284/railway` |
| **LAST COMPLETED** | **Phase 5.1 Voice-Activated Quick Edits (2026-05-23):** Полный эпик Context-Aware Quick Edits реализован и задеплоен. Детали — см. строки Phase 5.0 и Phase 5.1 в Секции 2. Commit `174b8f7`. |
| **BLOCKER** | None. |
| **NEXT ACTION** | 1. Использовать `account_type`/`sub_type` в `classifyAccountGroup()` вместо имя-эвристики. 2. E2E тест перевода: убедиться что внутренний перевод не ломает баланс. 3. Расширить голосовые команды (edit_last через голос уже работает). 4. Всегда писать изменения в workflow_state.md в конце сессии. |
...
| Phase 5.0 Context-Aware Quick Edits (текст) | ✅ DEPLOYED | **Сессия 2026-05-22–23.** Пользователь пишет «Измени сумму» — бот мгновенно находит последнюю транзакцию и открывает нужный пикер, минуя промежуточные клики. **Phase 5.0-A (Regex Router & Sentinel):** `packages/shared/src/command-router.ts` — 4 новых NavCommand (`edit_amount`, `edit_category`, `edit_account`, `edit_type`); строгий порядок паттернов (перед `edit_last`); `checkNumber: false`. `apps/background-workers/src/workers/voice-parse.worker.ts` — exhaustiveness check: 4 новых case → `return null`. **Phase 5.0-B (Core UI Delegation):** `apps/telegram-bot/src/routes/webhook.route.ts` — функция `handleQuickEditField()`: Send-First Bridge (новое сообщение, не edit); Redis bridge `midas:tx:edit:amt:{uid}:{cid}` → `{txId}:{sentMsgId}:s`; удаление success card перед пикером через `midas:last_confirmed` / `midas:am`; backSuffix `:s` для standalone context. **Phase 5.0-C (Edge Cases & UX Polish):** справка обновлена секцией «✏️ БЫСТРОЕ РЕДАКТИРОВАНИЕ»; success card удаляется перед открытием любого пикера. Commits: серия до `3e74bc4`. |
| Phase 5.1 Voice-Activated Quick Edits | ✅ DEPLOYED | **Сессия 2026-05-23 09:00–13:35.** Голосовые команды «измени сумму/категорию/счёт/тип» открывают те же пикеры, что и текстовые. Трёхслойная архитектура (Layer 1 SQL / Layer 2 @midas/shared builders / Layer 3 transport). **Phase 5.1-Pre (Shared UI Service Extraction):** `packages/shared/src/quick-edit-ui.ts` (NEW) — 4 pure builder-функции: `buildQuickEditAmountKb`, `buildQuickEditCategoryKb`, `buildQuickEditAccountKb`, `buildQuickEditIntentKb` + типы `QECategoryInfo`, `QEAccountInfo`. `packages/shared/src/category-emoji.ts` (NEW) — `getCategoryEmoji()`. `packages/shared/src/html-escape.ts` (NEW) — `escapeHtml()`. `packages/shared/src/index.ts` (MODIFY) — реэкспорты. `apps/telegram-bot/src/utils/category-emoji.ts` + `html-escape.ts` (MODIFY) — shimы → реэкспорты. turbo 5/5 build ✅. **Phase 5.1-A (Webhook Migration):** `apps/telegram-bot/src/routes/webhook.route.ts` — `handleQuickEditField()` рефакторинг: ~190 строк → ~60 строк; 4 inline блока заменены вызовами shared builders; транспортный слой (smk, Redis bridge) нетронут. **Phase 5.1-B (Voice Worker Integration):** `apps/background-workers/src/workers/voice-parse.worker.ts` — `sendNewMessage()` хелпер (новое сообщение для amt); `deleteSuccessCardW()` хелпер (удаление success card); 4 case реализованы: edit_amount → `sendNewMessage` + Redis bridge + `__SENT__` sentinel; edit_category/edit_account/edit_type → `return { text, keyboard }` → caller делает `editStatusMessage`; caller обновлён: проверяет `navResult.text === '__SENT__'` и удаляет statusMessage. tsc 0 errors, turbo 5/5. **Phase 5.1-C (Regex Hardening):** `packages/shared/src/command-router.ts` — все 4 edit_* блока усилены: `(?![а-яё])` negative lookahead блокирует прошедшее время (изменил/поменяла/сменил → null); `[,.\s]+.*` принимает Whisper-пунктуацию между глаголом и существительным. Commit `174b8f7` pushed to origin/main. turbo 5/5 ✅, 0 TS errors ✅. |
| Phase 3.3 Excel Sheet0/Sheet1 Polish | ✅ DEPLOYED | **Сессия 2026-05-22 09:52–10:44.** Серия точечных исправлений по обратной связи пользователя. **Sheet0 (Сводка):** Переводы убраны из СВОДКИ ЗА ПЕРИОД, ИТОГА ЗА ПЕРИОД и СВОДКИ ПО ВАЛЮТАМ — переводы не P&L. Колонки C/D в СВОДКЕ ПО ВАЛЮТАМ: 28+28 → 18+14. «Нетто за период» заголовок и данные: по левому краю с отступом. «Нетто» итог (col E): по центру. **Sheet1 (Транзакции):** Столбец «Сумма»: явный `+` для доходов (numFmt `+#,##0.## "USD";-#,##0.## "USD"`). «Остаток» при переводе: фиолетовый фон (не красный). «Остаток» при переводе: текст `{SourceAccount} ↓ {balance} {cur}` + cell.note `Откуда: {src} / Куда: {tgt}`. **Правило:** все правки через Node.js `.cjs`-скрипты (не PowerShell). Commits: `979ec20`, `df41080`, `e05c590`, `c2251fb`, `d081073`, `2f886f2`. |
...
| 2026-05-15 14:37 | **Balance Visual Redesign chevron fix (commit fbdfa0a). DEPLOYED.** balance.service.ts: замена 🟢/🔴 на chevron, убран пробел у минуса. Railway auto-deploy. |
| 2026-05-15 19:00 | **fix(stt): Normalize crypto tickers (f4eb9c1). DEPLOYED.** ai-parse.worker.ts: юздт->USDT, ефир->ETH. Railway auto-deploy. |
| 2026-05-15 19:05 | **fix(ux): Auto-delete Otmeneno card (fb5f140). DEPLOYED.** webhook.route.ts: midas:dead_card cleanup при любом сообщении. |
| 2026-05-15 19:10 | **fix(ux): Delete dead card + voice status on new preview (3696042). DEPLOYED.** notifications.worker.ts: оба сообщения удаляются. |
| 2026-05-15 20:00 | **feat(draft): 2-level category picker (f87393a). DEPLOYED.** webhook.route.ts: Screen1: AI-hint + [🛒Жизнь][💼Бизнес]. Screen2: плоский список с emoji (28 категорий). clar:cat:{catId}:{draftId}. |
| 2026-05-15 20:10 | **fix(cat): Remove без-категории, smart flatten (17cc9a9). DEPLOYED.** Удалена «Без категории», auto-flatten при <=6 категориях, fix двойных emoji в заголовке. |
| 2026-05-15 20:15 | **db: Merge Raznoye->Drugoye (60cb4d9). DEPLOYED.** Миграция UPDATE+DELETE по всем workspace. Railway auto-deploy. |
| 2026-05-15 20:20 | **fix(cat): Backfill 28 default categories (63cd4a3). DEPLOYED.** Вставка 28 канонических категорий. AI-hint excluded from flat list. |
...
| 2026-05-16 10:00 | **feat(voice): xAI Grok STT pipeline (588e038). DEPLOYED.** voice-parse.worker.ts + groq-stt.ts: голос -> STT -> ai-parse. BullMQ voice-queue. Railway auto-deploy. |
| 2026-05-16 10:30 | **fix(balance): N26/Revolut redesign (ecf7e63). DEPLOYED.** Compact layout, Title Case, CCY_SYMBOL 17 валют, icon-only nav. tsc 0. |
| 2026-05-16 14:00 | **fix(onboard): Remove Vse Scheta, fix double-currency header (b9f7fee). DEPLOYED.** После добавления счёта -> currency-aware single card. Провайдер-иконки. Railway auto-deploy. |
...
| 2026-05-16 20:00 | **feat: type-aware currency picker for tx drafts (8537e8a). DEPLOYED.** Пикер фильтрует валюты по типу транзакции. Railway auto-deploy. |
| 2026-05-17 11:00 | **feat(sprint-0): Professional export suite 2.0 (affbe87). DEPLOYED.** excel-export.service.ts: 5 листов (Сводка, Транзакции, Счета, Категории, По месяцам). Unified UX: период -> счёт -> формат. midas:exp:params Redis key. Audit trail footer. tsc 0. |
| 2026-05-18 10:53 | **feat(export): Professional Export Suite 2.0 — Phase 2 Этап 1 (commits affbe87, 2bf24d2). DEPLOYED.** **excel-export.service.ts**: 5-листовый Excel (1-Сводка: workspace/период/счёт + KPI блок; 2-Транзакции: все колонки с running balance; 3-Счета: балансы; 4-Категории: аналитика; 5-По месяцам: monthly breakdown). Единый UX: 3 шага — период (Текущий месяц / 3 месяца / Год / Всё время) -> счёт (все / конкретный) -> формат (xlsx / csv). Redis key midas:exp:params:{userId}:{chatId} хранит выбранный период + accountId между шагами. Audit trail footer на листе Сводка. **webhook.route.ts — 3 бага исправлены:** (1) Шаг 3 показывал raw ULID -> теперь «Bybit · USDT» (резолв имени через getWorkspaceAccountsWithBalances); (2) Имя файла всегда был текущий месяц -> теперь соответствует периоду (MIDAS_Report_2026-04.xlsx, MIDAS_Report_2026-05_3m.xlsx, MIDAS_Report_all.xlsx); (3) Confirmation step показывал только период -> теперь период + счёт. **settings-advanced.service.ts**: exportTransactionsCSV() расширена параметрами dateFrom, dateTo, accountId — SQL фильтрует через DATE() и account_id =  (параметризованный запрос, без SQL-инъекций). **phase-2.0-pitch.md секция 6 переписана**: UX-preview 3-шагового диалога, 5 листов с конкретными колонками, таблица визуального стандарта (header #1A3C5E, accent #2E7D32, warning #E65100, разделитель тысяч = пробел, шрифт Calibri 11pt). tsc 0 ошибок. Railway auto-deploy. |
| 2026-05-15 08:00 | **feat(roles): Simplify to 2-state normal/primary toggle (750d1ba). DEPLOYED.** Упрощена система ролей аккаунта до 2 состояний. fix(roles): исправлены toast-иконки в set_role callback (5a8d74e). |
| 2026-05-15 08:30 | **feat(balance): Multi-currency UX overhaul Phase B-2+ (392c177). DEPLOYED.** Полная переработка многовалютного баланса. V2 redesign — dot notation, multi-currency cards, sub-accounts (c3f2334). |
| 2026-05-15 09:00 | **feat(balance): Phase 9 — auto-select primary account for AI parse (1ca5ab1). DEPLOYED.** ai-parse автоматически выбирает основной счёт. fix v3/v4: RLS-safe SQL через workspaces table (378b976, 1bd5ae8). |
| 2026-05-15 10:00 | **feat(voice): xAI Grok STT — crash fixes (e77cf67, 7fa5b9f). DEPLOYED.** Удалён BullMQ limiter (вызывал crash при старте). Исправлен frozen status msg и colon-in-jobId crash в BullMQ. |
| 2026-05-15 11:00 | **feat(voice): Clean chat UX (098c620, 62f94fd). DEPLOYED.** Удаление голосового сообщения + отслеживание/очистка error messages. fix: удаление «⏳ Распознаю…» когда gate отклоняет новый голосовой ввод. |
| 2026-05-15 12:00 | **ui(balance): Professional fintech balance redesign (f3780b5..e418b6a). DEPLOYED.** Bloomberg+Coinbase hybrid renderer. Backward compat для старой 💰 кнопки. Убраны <pre> блоки, clean HTML renderer. Мультивалюта на одной строке через / разделитель. |
| 2026-05-15 19:30 | **feat(ai): Phase 2.7 — spoken-number normalization + stop-words (4a38176). DEPLOYED.** Нормализация устных чисел («сто» → 100), stop-words filter, украинские глаголы. fix(draft): кнопка Back в currency picker возвращает на confirm card (45b3ac7). |
| 2026-05-16 08:00 | **feat: success screen v2, /help cheat sheet (153b44f). DEPLOYED.** Новый success screen. Команда /help с cheat sheet. Удалена «Базовая валюта» из settings UI (c32186d, 71087838). |
| 2026-05-16 08:30 | **fix(balance): fiat/crypto separation in currency picker (691da2d..1e30c92). DEPLOYED.** Фильтрация пикера по типу аккаунта (fiat/crypto). Добавлен SGD в fiat presets (полная 4×3 grid). Строгое разделение fiat/crypto. |
| 2026-05-16 09:00 | **feat(ux): Receipt-style account success screen (7395d9d). DEPLOYED.** Redesign success screen в стиле банковского чека. fix: всегда используется русский язык в примерах валют (0f8171e). |
| 2026-05-16 09:30 | **fix(balance): add_currency UX + back navigation (45bf6b5, a4faf03). DEPLOYED.** После add_currency показывается multi-card (не список). Исправлена nav кнопка Back из single-settings → multi-card. |
| 2026-05-16 10:00 | **fix: Skrill icon, e-wallet PROVIDER_ICONS (fba0f2b). DEPLOYED.** 📱 иконка для Skrill. 3-example blockquote format для ввода. |
| 2026-05-16 10:30 | **feat: reminder keyboard — account context + account picker (0d90ef3, 309b39f). DEPLOYED.** Напоминание показывает контекст счёта. Reminder card всегда показывает account picker (один тап = выбор + подтверждение). |
| 2026-05-16 11:00 | **feat(phase-2.6): reminder screen mirror + chat hygiene (da1f4ce, cc26652). DEPLOYED.** Зеркалирование reminder screen. Используется real account_currency в reminder keyboard. fix: rename migration для корректного timestamp (63d6ef8). |
| 2026-05-16 11:30 | **fix(settings): clear tz_srch Redis key on Back (77059cc). DEPLOYED.** Очистка midas:tz_srch при нажатии Back — предотвращает зависание timezone search. |
| 2026-05-16 12:00 | **feat: transfer deducts from account balance D3 (bdfb96c). DEPLOYED.** Переводы корректно вычитают сумму с баланса счёта (D3 semantic change). fix: initial success card missing balance + duplicate currency in child accounts (8c0de4b). |
| 2026-05-16 12:30 | **fix: unify debt icons 🤝/🤲 (ac4f97b). DEPLOYED.** Единые иконки для debt-транзакций на всех UI поверхностях. feat: показ badge архивного аккаунта на transaction card (a88e3c2). |
| 2026-05-16 13:00 | **feat: guarantee item_name via L2 fallback (4d13c71). DEPLOYED.** buildItemName() — L2 fallback гарантирует item_name для каждой транзакции. |
| 2026-05-16 13:30 | **feat: professional account picker — icons, balance, cross-currency grouping (c4be32a). DEPLOYED.** Профессиональный пикер счетов: иконки провайдеров, балансы, группировка по валюте. fix: broken unicode escape in ed:field_acc (034af02). |
| 2026-05-16 14:30 | **fix: onboarding flow for users with no accounts (fd17266, a4a261b). DEPLOYED.** Показывает create-account prompt вместо старых confirm buttons. Onboarding flow для returning users без счетов (после reset). |
| 2026-05-16 15:00 | **feat: 2-level category picker with emoji (e4084644, 77b10c1). DEPLOYED.** Второй заход на 2-level category picker (в background-workers контексте). Текущая категория перенесена в header text вместо кнопки. fix: всегда показывается account+balance block на confirmation card (cc84678). |
| 2026-05-17 08:00 | **feat: professional Excel export 4 sheets (81ef834). DEPLOYED.** Первая итерация Excel export: 4 листа (Transactions, Accounts, Categories, Monthly). fix: transfer ru locale, executor heuristic, debit fallback (aade176). |
| 2026-05-17 09:00 | **feat: running balance column in Excel export (16d7e21). DEPLOYED.** Колонка «Остаток на счёте» в Excel (нарастающий итог). fix: per-currency summary + signed Выплачено всегда отображается (0ba6ef5). |
...
| 2026-05-18 18:00 | **fix(excel): Sheet1 Транзакции — удаление избыточных колонок + числовые суффиксы валют (bb3904c). DEPLOYED.** excel-export.service.ts: удалены колонки «Тип транзакции» и «Категория группа» из листа Транзакции — оставлены только значимые для журнального стиля. Числовые суффиксы валют: суммы отображаются в формате «15 400 USDT» без символов ₴/$/€ в отдельной колонке — плотный читаемый формат. tsc 0 ошибок. Railway auto-deploy. |
| 2026-05-18 18:15 | **fix(excel): TS2532 — Object possibly undefined (b5e71cd). DEPLOYED.** excel-export.service.ts: исправлена TypeScript-ошибка TS2532 при обращении к объектам, которые могут быть undefined. Добавлены опциональные цепочки и дефолтные значения. tsc 0 ошибок. Railway auto-deploy. |
| 2026-05-18 18:30 | **fix(excel): Column E always visible + % centered (76c3def). DEPLOYED.** excel-export.service.ts: колонка E (доля %) в блоке ТОП РАСХОДОВ теперь всегда отображается даже при нулевых данных; выравнивание центрированное для числовых значений % (профессиональный финтех-стандарт). tsc 0 ошибок. Railway auto-deploy. |
| 2026-05-18 18:45 | **fix(excel): 5 visual fixes — signs, grey fill, headers, center alignment, ИТОГО removal (ac7489a). DEPLOYED.** excel-export.service.ts: (1) Исправлены знаки в колонке Выплачено/Получено — outflow всегда отображается со знаком «−» (стандарт банковской выписки); (2) Grey fill (#F5F5F5) для чётных строк в секции СВОДКА ПО ВАЛЮТАМ; (3) Заголовки колонок: унифицированы по всем блокам Sheet0; (4) Центрирование числовых ячеек в балансовых строках; (5) Блок «ИТОГО» удалён из Sheet1 Транзакции — переход к чистому журнальному формату без суммарных строк в теле листа. tsc 0 ошибок. Railway auto-deploy. |
| 2026-05-18 19:30 | **fix(excel): Sheet1 Audit-grade polish — exchange rate col L + rate center + Суммы C-E + balance center (2f4b88c). DEPLOYED.** excel-export.service.ts: (1) Колонка L (Курс): показывает описательную конвертацию только при кросс-валютных операциях (когда currency ≠ base_currency) — формат «1 USD = 38.5 UAH»; при одновалютных операциях ячейка пуста (не «1»); (2) Курс выравнен по центру; (3) Колонки C–E (Приход / Расход / Сальдо) — унифицированный smartNumFmt с NBSP-разделителем тысяч, без trailing zeros; (4) Колонка «Остаток» (running balance) — центрирована по горизонтали для удобства чтения. tsc 0 ошибок. Commit 2f4b88c (HEAD). Railway auto-deploy. |
