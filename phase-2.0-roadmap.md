...
| 7 — Умный ввод | 3 | 0 | ⏳ Запланирован |
| 8 — Голос | 2 | 0 | ⏳ Запланирован |
| **Итого** | **39** | **33** | **85%** |
...
> **Цель — Этап 1:** Принимать голосовые сообщения, транскрибировать через Whisper, обрабатывать через существующий Intent Router. Обработка технических ошибок распознавания. Голосовые команды (без транзакций).

> ⚠️ **Этап 2 (НЕ сейчас):** Vision AI (фото чеков и скриншотов), авто-курсы из API — следующий этап развития.

---

### Задача 8.1 — Голосовые транзакции (Whisper)
- **Статус:** `[ ]`
- **Источник:** product-roadmap.md, Phase 2.1
- **Суть:** Голосовое сообщение → Whisper API → транскрипция → существующий AI-парсер → черновик → подтверждение кнопками. Весь happy path идентичен текстовому вводу.
...
**Технические детали:**
- Новый worker: `voice-parse.worker.ts` в `background-workers/`
- Интеграция: `openai` SDK (`whisper-1` модель) или `@anthropic-ai/sdk` (если Claude Audio)
- Telegram: `voice` и `audio` message types → скачать через `getFile` → Buffer → Whisper
- После транскрипции → тот же `ai-parse.worker.ts` pipeline (без изменений)
- Redis key: `midas:voice:{userId}:{chatId}` — счётчик неудачных попыток (TTL 10 мин)
- Callback namespace: `vc:retry` — попробовать снова, `vc:text` — перейти в текст, `vc:cancel` — отмена
- Миграция: не нужна
