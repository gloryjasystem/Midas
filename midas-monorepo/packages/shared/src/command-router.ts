...
const COMMAND_PATTERNS: CommandPattern[] = [
  ...
  //
  // Safety design:
  //   ✅ Only IMPERATIVE verbs are used (измени, поменяй, смени, исправь, редактир*)
  //   ✅ Each pattern requires BOTH the verb AND the noun with \s.* between them
  //   ✅ checkNumber: false — "измени сумму на 500" is still quick-edit, not AI parse
  //   ❌ Ambiguous phrases ("другую сумму", "это был доход") are intentionally omitted
  //      because they appear naturally in new-transaction descriptions.
  {
    cmd: 'edit_amount',
    patterns: [
      /измени(?:ть)?\s.*сумм/i,       // "измени сумму" / "изменить сумму последней"
      /поменя(?:ть|й)?\s.*сумм/i,     // "поменяй сумму" / "поменять сумму"
      /смени(?:ть)?\s.*сумм/i,        // "смени сумму" / "сменить сумму"
      /редактир\w*\s.*сумм/i,         // "редактируй сумму" / "редактировать сумму"
      /исправ(?:ить|ь)?\s.*сумм/i,    // "исправь сумму" / "исправить сумму"
    ],
    checkNumber: false,
  },
  {
    cmd: 'edit_category',
    patterns: [
      /измени(?:ть)?\s.*категори/i,
      /поменя(?:ть|й)?\s.*категори/i,
      /смени(?:ть)?\s.*категори/i,
      /редактир\w*\s.*категори/i,
      /исправ(?:ить|ь)?\s.*категори/i,
    ],
    checkNumber: false,
  },
  {
    cmd: 'edit_account',
    patterns: [
      /измени(?:ть)?\s.*сч[её]т/i,
      /поменя(?:ть|й)?\s.*сч[её]т/i,
      /смени(?:ть)?\s.*сч[её]т/i,
      /редактир\w*\s.*сч[её]т/i,
      /исправ(?:ить|ь)?\s.*сч[её]т/i,
    ],
    checkNumber: false,
  },
  {
