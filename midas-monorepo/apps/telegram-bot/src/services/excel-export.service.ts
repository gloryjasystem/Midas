...
export async function exportTransactionsExcel(
  ...
  buildSheet6PayeeAnalytics(wb, rows, usdRates);
...
// ═══════════════════════════════════════════════════════════════
// Sheet 6: «Контрагенты» — Payee Analytics
// ═══════════════════════════════════════════════════════════════

function buildSheet6PayeeAnalytics(
  wb: ExcelJS.Workbook,
  rows: TxRow[],
  usdRates: Map<string, number>,
): void {
  ...
  ws.mergeCells(1, 1, 1, TOTAL_COLS);
  const titleCell = ws.getCell(1, 1);
  titleCell.value = 'MIDAS · Аналитика по контрагентам (≈ USD)';
  titleCell.font  = { bold: true, size: 13, name: 'Calibri', color: { argb: `FF${C_COL_HDR_FG}` } };
  titleCell.fill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C_HEADER_BG}` } };
  titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getRow(1).height = 32;
  ...
  // Aggregate by payee (person_name or executor heuristic)
  type PayeeEntry = {
    count: number;
    expenseUsd: number;
    incomeUsd: number;
  ...
  const payeeMap = new Map<string, PayeeEntry>();

  for (const row of rows) {
    // Extract payee name
    let payee = row.person_name ?? '';
    if (!payee && row.item_name) {
      const words = row.item_name.trim().split(/\s+/);
      const lastCapital = words.reverse().find(w => w.length > 2 && /^[А-ЯA-Z]/.test(w));
      payee = lastCapital ?? '';
    }
    if (!payee) continue; // skip anonymous transactions
    ...
    const entry = payeeMap.get(payee) ?? { count: 0, expenseUsd: 0, incomeUsd: 0, lastDate: txDate };
    entry.count++;
    if (row.transaction_intent === 'expense') {
      entry.expenseUsd += usd;
    } else if (row.transaction_intent === 'income') {
      entry.incomeUsd += usd;
    }
    if (txDate > entry.lastDate) entry.lastDate = txDate;
    payeeMap.set(payee, entry);
  }

  // Sort by total volume DESC (expense + income)
  const sorted = [...payeeMap.entries()]
    .sort(([, a], [, b]) => (b.expenseUsd + b.incomeUsd) - (a.expenseUsd + a.incomeUsd));
  ...
  const DATA_START = 3;
  let rn = DATA_START;
  let grandOps = 0, grandExp = 0, grandInc = 0;

  for (const [name, p] of sorted) {
    const net = p.incomeUsd - p.expenseUsd;
    const avgChk = p.count > 0 ? (p.expenseUsd + p.incomeUsd) / p.count : 0;
    const bgArgb = rn % 2 === 0 ? `FF${C_ROW_ODD}` : 'FFFFFFFF';
    const fillBg = { type: 'pattern' as const, pattern: 'solid' as const, fgColor: { argb: bgArgb } };
    ws.getRow(rn).height = 18;

    grandOps += p.count;
    grandExp += p.expenseUsd;
    grandInc += p.incomeUsd;
    ...
    sc(1, rn - DATA_START + 1, { align: 'center', color: 'FF888888' });
    sc(2, name, { bold: true, color: 'FF1A3C5E' });
    sc(3, p.count, { align: 'center' });
    sc(4, p.expenseUsd, { align: 'right', numFmt: '#,##0.00',
      color: p.expenseUsd > 0 ? `FF${C_EXPENSE}` : 'FF888888' });
    sc(5, p.incomeUsd, { align: 'right', numFmt: '#,##0.00',
      color: p.incomeUsd > 0 ? `FF${C_INCOME}` : 'FF888888' });
    sc(6, net, { align: 'right', numFmt: '#,##0.00', bold: true,
      color: net >= 0 ? `FF${C_INCOME}` : `FF${C_EXPENSE}`,
      fill: { type: 'pattern', pattern: 'solid',
        fgColor: { argb: net >= 0 ? 'FFE8F8F5' : 'FFFDEDEC' } } });
    sc(7, avgChk, { align: 'right', numFmt: '#,##0.00', color: 'FF888888' });
    sc(8, fmtDate(p.lastDate), { align: 'center', color: 'FF666666' });

    rn++;
  }

  // Grand Total
  if (sorted.length === 0) return; // no payees to show
  ...
  const g1 = ws.getCell(gtRow, 1);
  g1.value = ''; g1.fill = gtFill;
  const g2 = ws.getCell(gtRow, 2);
  g2.value = `ИТОГО (${String(sorted.length)} контр.)`; g2.font = gtFont('FFB0C8E0', 10); g2.fill = gtFill;
  g2.alignment = { horizontal: 'right', vertical: 'middle' };
  const g3 = ws.getCell(gtRow, 3);
  g3.value = grandOps; g3.font = gtFont('FFB0C8E0'); g3.fill = gtFill;
  g3.alignment = { horizontal: 'center', vertical: 'middle' };
  const g4 = ws.getCell(gtRow, 4);
  g4.value = grandExp; g4.numFmt = '#,##0.00';
  g4.font = gtFont(grandExp > 0 ? 'FFE57373' : 'FFB0C8E0', 10); g4.fill = gtFill;
  g4.alignment = { horizontal: 'right', vertical: 'middle' };
  const g5 = ws.getCell(gtRow, 5);
  g5.value = grandInc; g5.numFmt = '#,##0.00';
  g5.font = gtFont(grandInc > 0 ? 'FF7DCEA0' : 'FFB0C8E0', 10); g5.fill = gtFill;
  g5.alignment = { horizontal: 'right', vertical: 'middle' };
  const grandNet = grandInc - grandExp;
  const g6 = ws.getCell(gtRow, 6);
  g6.value = grandNet; g6.numFmt = '#,##0.00';
  g6.font = gtFont(grandNet >= 0 ? 'FF7DCEA0' : 'FFE57373', 10); g6.fill = gtFill;
  ...
  ws.mergeCells(fnRow, 1, fnRow, TOTAL_COLS);
  const fn = ws.getCell(fnRow, 1);
  fn.value = 'Контрагенты определяются из поля «Исполнитель» · Анонимные транзакции не учитываются · Ø чек = (Расходы + Доходы) / Операций';
  fn.font  = { size: 7, italic: true, name: 'Calibri', color: { argb: 'FFAAAAAA' } };
  fn.fill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C_GREY_BG}` } };
  fn.border = { top: { style: 'thin', color: { argb: `FF${C_TBL_BORDER}` } } };
  ...
  ws.properties.tabColor = { argb: 'FF8E44AD' }; // purple tab
}
