  ...
  fastify.post<{ Body: TelegramUpdate }>('/webhook', async (request, reply) => {
    ...
    if (update.callback_query) {
        ...
        try {
          if (acCmd.cmd === 'open') {
            ...
            if (!rawState) {
              ...
              // ── Handle: same name + same currency → already exists ─────
              if (res.status === 'already_exists') {
                const dupState: AccountOnboardState = {
                  step: 'rename_for_dup',
                  accountType: state.accountType,
                  name: state.name,
                  currency: acCmd.code,
                  linkedDraftId: state.linkedDraftId,
                  walletSubtype: state.walletSubtype,
                };
                await redisConnection.set(acKey, JSON.stringify(dupState), 'EX', ONBOARD_STATE_TTL_SEC);

                const dupMsg =
                  `⚠️ Счёт <b>${escapeHtml(accountName)} · ${escapeHtml(acCmd.code)}</b> уже существует.\n\n` +
                  `<i>Введите другое название или нажмите кнопку ниже.</i>`;
                const dupKb = {
                  inline_keyboard: [
                    [{ text: `✅ Использовать ${accountName} ${acCmd.code}`, callback_data: `ac:use:${res.existingAccountId}` }],
                    [{ text: '✖️ Отмена', callback_data: 'ac:fin' }],
                  ],
                };
                if (acMsgId) void editMessageText(chatId, acMsgId, dupMsg, dupKb);
                else void upsertBotMessage(telegramUserId, chatId, dupMsg, dupKb);
                await reply.status(200).send({ ok: true });
                return;
              }

              // ── Handle: child created OR normal create OR auto-suffix ──
              const wasChild = res.status === 'created_as_child';
              const wasAutoSuffixed = res.status === 'created_with_suffix';
              const effectiveName = (wasChild || wasAutoSuffixed) ? res.finalName! : accountName;
              ...
              // Balance prompt — context-aware message
              const balPrompt = wasChild
                ? `✅ Добавлена валюта <b>${escapeHtml(acCmd.code)}</b> к счёту <b>${escapeHtml(accountName)}</b>\n\n` +
                  `Счёт: <b>${escapeHtml(effectiveName)}</b>\n\n💰 Введите начальный баланс\nили нажмите «Пропустить»:`
                : wasAutoSuffixed
          ...
          } else if (acCmd.cmd === 'use_existing') {
            // User chose to use existing account (same name + same currency duplicate)
            const rawStateUse = await redisConnection.get(acKey);
            let linkedDraftIdUse: string | undefined;
            let useCurrency = 'USD';
            if (rawStateUse) {
              try {
                const s = JSON.parse(rawStateUse) as AccountOnboardState;
                linkedDraftIdUse = s.linkedDraftId;
                useCurrency = s.currency ?? 'USD';
              } catch { /* ignore */ }
            }
            await redisConnection.del(acKey);

            if (linkedDraftIdUse) {
              // Link existing account to the draft and show confirmation / picker
              await showDraftAfterAccountCreation(
                acResolved.workspaceId, acResolved.userId,
                telegramUserId, chatId,
                linkedDraftIdUse, acCmd.accountId, useCurrency,
              );
              request.log.info({ msg: '[midas:bot:webhook] ac: use_existing — linked to draft', workspaceId: acResolved.workspaceId });
            } else {
              // Standalone flow — just show confirmation
              if (acMsgId) {
                void editMessageText(chatId, acMsgId, '✅ Счёт выбран.', buildAfterCreateKeyboard());
              } else {
                void upsertBotMessage(telegramUserId, chatId, '✅ Счёт выбран.', buildAfterCreateKeyboard());
              }
            }
      ...
      if (callbackData.startsWith('tx:catg:')) {
        ...
        const catgSub    = catgParts[2] ?? '';   // 'life' | 'biz' | 'back' | 'mine'
        const catgTxId   = catgParts[3] ?? '';
        const catgFrom   = catgParts[4];
        const catgSf     = catgFrom ? `:${catgFrom}` : '';

        if (!/^[0-9A-Z]{26}$/.test(catgTxId) || !['life', 'biz', 'back', 'mine'].includes(catgSub)) {
          await answerCallbackQuery(cq.id);
          await reply.status(200).send({ ok: true });
          return;
        }
        ...
        try {
          ...
          if (catgSub === 'back') {
            ...
            const currentCatBack = cardForBack
              ...
              : null;
            const standardCatsBack = allCatgTx.filter(c => !c.is_custom);
            const customCatsBack   = allCatgTx.filter(c => c.is_custom === true);
            const lifeCatsBack = standardCatsBack.filter(c => c.group === 'Жизнь');
            const bizCatsBack  = standardCatsBack.filter(c => c.group === 'Бизнес');
            const useFlatBack  = standardCatsBack.length <= 6 || lifeCatsBack.length === 0 || bizCatsBack.length === 0;
            const backRows: { text: string; callback_data: string }[][] = [];

            // ✨ Current category as hint (top row)
            if (currentCatBack) {
              backRows.push([{
                text: `✨ ${getCategoryEmoji(currentCatBack.name, currentCatBack.icon)} ${currentCatBack.name}`,
                callback_data: `tx:c:cat:${catgTxId}:${currentCatBack.id}${catgSf}`,
              }]);
            }
            if (useFlatBack) {
              const catsToShow = currentCatBack ? standardCatsBack.filter(c => c.id !== currentCatBack.id) : standardCatsBack;
              for (let i = 0; i < catsToShow.length; i += 2) {
                ...
                backRows.push(b ? [btnA, { text: `${getCategoryEmoji(b.name, b.icon)} ${b.name}`, callback_data: `tx:c:cat:${catgTxId}:${b.id}${catgSf}` }] : [btnA]);
              }
            } else {
              backRows.push([
            ...
            if (customCatsBack.length > 0) {
              backRows.push([{ text: `⭐ Мои (${String(customCatsBack.length)})`, callback_data: `tx:catg:mine:${catgTxId}${catgSf}` }]);
            }
            backRows.push([{ text: '✏️ Создать', callback_data: `cc:new:tx:${catgTxId}${catgSf}` }]);
            ...
            // ── ⭐ Мои категории ───────────────────────────────────────────
            const customOnly = allCatgTx.filter(c => c.is_custom === true);
            const mineRows: { text: string; callback_data: string }[][] = [];
            if (customOnly.length === 0) {
              mineRows.push([{ text: '⚠️ Нет кастомных категорий', callback_data: `tx:catg:back:${catgTxId}${catgSf}` }]);
            } else {
              for (let i = 0; i < customOnly.length; i += 2) {
                const a = customOnly[i]!;
                const b = customOnly[i + 1];
                const iconA = getCategoryEmoji(a.name, a.icon);
                const btnA = { text: `${iconA} ${a.name}`, callback_data: `tx:c:cat:${catgTxId}:${a.id}${catgSf}` };
                mineRows.push(b
                  ? [btnA, { text: `${getCategoryEmoji(b.name, b.icon)} ${b.name}`, callback_data: `tx:c:cat:${catgTxId}:${b.id}${catgSf}` }]
                  : [btnA]);
              }
            }
            mineRows.push([{ text: '✏️ Создать', callback_data: `cc:new:tx:${catgTxId}${catgSf}` }]);
            mineRows.push([{ text: '◀️ К группам', callback_data: `tx:catg:back:${catgTxId}${catgSf}` }]);
            if (txCatgMsgId) void editMessageText(chatId, txCatgMsgId, '<b>⭐ Мои категории:</b>', { inline_keyboard: mineRows });
          } else {
      ...
      if (callbackData.startsWith('tx:')) {
        ...
        try {
          if (txCmd.cmd === 'cancel') {
            ...
            // Phase 4.0: Full picker — AI hint + groups + ⭐ Мои + ✏️ Создать
            // Mirrors the draft category picker (clar:cat flow) for visual consistency.
            const sf = txCmd.from ? `:${txCmd.from}` : '';
            ...
            if (allCatsFC.length === 0) {
              ...
              const standardCats   = allCatsFC.filter(c => !c.is_custom);
              const lifeCatsFC     = standardCats.filter(c => c.group === 'Жизнь');
              const bizCatsFC      = standardCats.filter(c => c.group === 'Бизнес');
              const customCatsFC   = allCatsFC.filter(c => c.is_custom === true);
              const useFlatFC      = standardCats.length <= 6 || lifeCatsFC.length === 0 || bizCatsFC.length === 0;
              const fcRows: { text: string; callback_data: string }[][] = [];

              // ── Текущая категория как ✨ AI-hint по аналогии с черновиком ───────────────
              if (currentCatFC) {
                fcRows.push([{
                  text: `✨ ${getCategoryEmoji(currentCatFC.name, currentCatFC.icon)} ${currentCatFC.name}`,
                  callback_data: `tx:c:cat:${txCmd.txId}:${currentCatFC.id}${sf}`,
                }]);
              }

              // ── Флет-лист или группы стандартных категорий ────────────────────
              if (useFlatFC) {
                const catsToShow = currentCatFC
                  ? standardCats.filter(c => c.id !== currentCatFC.id)
                  : standardCats;
                for (let i = 0; i < catsToShow.length; i += 2) {
                  const a = catsToShow[i]!;
                  const b = catsToShow[i + 1];
                  const btnA = { text: `${getCategoryEmoji(a.name, a.icon)} ${a.name}`, callback_data: `tx:c:cat:${txCmd.txId}:${a.id}${sf}` };
                  fcRows.push(b ? [btnA, { text: `${getCategoryEmoji(b.name, b.icon)} ${b.name}`, callback_data: `tx:c:cat:${txCmd.txId}:${b.id}${sf}` }] : [btnA]);
                }
              } else {
                fcRows.push([
              ...
              // ── ⭐ Мои (N) ────────────────────────────────────────────────────
              if (customCatsFC.length > 0) {
                fcRows.push([{
                  text: `⭐ Мои (${String(customCatsFC.length)})`,
                  callback_data: `tx:catg:mine:${txCmd.txId}${sf}`,
                }]);
              }

              // ── ✏️ Создать + ◀️ Назад ─────────────────────────────────
              fcRows.push([{ text: '✏️ Создать', callback_data: `cc:new:tx:${txCmd.txId}${sf}` }]);
      ...
      if (callbackData.startsWith('st:')) {
        ...
        try {
          ...
          // ── Phase 2.0: advanced settings handlers ──
            ...
            // Build reminder count label (Phase 7.1)
            let remLabel = '· Выкл';
            try {
              const { getReminderCount } = await import('../services/reminder.service.js');
              const cnt = await getReminderCount(stResolved.workspaceId, stResolved.userId);
              if (cnt > 0) remLabel = `· ${String(cnt)} актив.`;
            } catch { /* table may not exist yet */ }
            ...
            const kb = {
              inline_keyboard: [
                ...
                [{ text: `📅 Напоминания  ${remLabel}`, callback_data: 'st:ntf:rem' }],
          ...
          // Phase 7.1: Reminders sub-menu (entry from notifications hub)
          else if (cmd.cmd === 'ntf_reminders') {
            try {
              const { getReminderListScreen } = await import('../services/reminder.service.js');
              const screen = await getReminderListScreen(stResolved.workspaceId, stResolved.userId);
              if (messageId) void editMessageText(chatId, messageId, screen.text, screen.keyboard);
            } catch {
              const text = '📅 <b>Напоминания</b>\n\nФункция скоро будет доступна.';
              const kb = { inline_keyboard: [[{ text: '🔙 Назад', callback_data: 'st:ntf' }]] };
              if (messageId) void editMessageText(chatId, messageId, text, kb);
            }
          }
...
Мы используем передовые технологии безопасности и строго соблюдаем конфиденциальность ваших данных.`;
      ...
      if (callbackData.startsWith('bud:')) {
        ...
        const remMsgId = String(cq.message?.message_id ?? '');
        const telegramUserId = String(cq.from.id);
        try {
          ...
          if (sub === 'list') {
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
          }
          else if (sub === 'add') {
            ...
            if (availableCats.length === 0) {
              if (remMsgId) void editMessageText(chatId, remMsgId, '💰 Все категории уже имеют лимиты.', {
              ...
              if (remMsgId) void editMessageText(chatId, remMsgId, '➕ <b>Новый лимит</b>\n\nВыберите категорию:', { inline_keyboard: rows });
            }
          }
          // bud:catg:life / bud:catg:biz / bud:catg:mine — category group drill-down
            ...
            if (catgSub === 'mine') {
              ...
              if (remMsgId) void editMessageText(chatId, remMsgId, '<b>⭐ Мои категории:</b>', { inline_keyboard: rows2 });
              ...
              rows2.push([{ text: '◀️ К группам', callback_data: 'bud:catg:back' }]);
              if (remMsgId) void editMessageText(chatId, remMsgId, `<b>${groupEmoji} ${groupName}:</b>`, { inline_keyboard: rows2 });
            }
          }
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, text, kb);
            // Set text intercept
            await redisConnection.set(
              `midas:bud:amt:${telegramUserId}:${chatId}`,
              `${categoryId}:${remMsgId}`,
              'EX', 120,
            );
          }
            ...
            if (detail) {
              ...
              if (remMsgId) void editMessageText(chatId, remMsgId, '⚠️ Лимит не найден.', {
                inline_keyboard: [[{ text: '🔙 Назад', callback_data: 'bud:list' }]],
              });
            }
          }
            ...
            if (detail) {
              ...
              if (remMsgId) void editMessageText(chatId, remMsgId, text, {
                inline_keyboard: [[{ text: '🔙 Отмена', callback_data: `bud:v:${limitId}` }]],
              });
              await redisConnection.set(
                `midas:bud:edit:${telegramUserId}:${chatId}`,
                `${limitId}:${remMsgId}`,
                'EX', 120,
              );
            }
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, '✅ Лимит удалён.', {
              inline_keyboard: [[{ text: '🔙 К лимитам', callback_data: 'bud:list' }]],
            });
          }
          else if (sub === 'close') {
            void deleteMessage(chatId, remMsgId);
          }
        } catch (err: unknown) {
          const errorClass = err instanceof Error ? err.constructor.name : 'UnknownError';
      ...
      if (callbackData.startsWith('sub:')) {
        ...
        try {
          ...
          if (sub === 'list') {
            ...
            await redisConnection.set(
              `midas:sub:name:${telegramUserId}:${chatId}`,
              remMsgId,
              'EX', 120,
            );
          }
            ...
            if (detail) {
              ...
              if (remMsgId) void editMessageText(chatId, remMsgId, '⚠️ Подписка не найдена.', {
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, '⏸ Подписка приостановлена.', {
              inline_keyboard: [
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, '▶️ Подписка возобновлена.', {
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, `🗑️ Удалить подписку «${name}»?\n\nЭто действие нельзя отменить.`, {
              inline_keyboard: [
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, '✅ Подписка удалена.', {
              inline_keyboard: [[{ text: '🔙 К подпискам', callback_data: 'sub:list' }]],
            });
          }
            ...
            if (detail) {
              ...
              await redisConnection.set(
                `midas:sub:eamt:${telegramUserId}:${chatId}`,
                `${recId}:${remMsgId}`,
                'EX', 120,
              );
            }
            ...
            const pendingKey = `midas:sub:freq:${telegramUserId}:${chatId}`;
            ...
            if (pendingData) {
              ...
              await redisConnection.set(
                `midas:sub:confirm:${telegramUserId}:${chatId}`,
            ...
            const confirmKey = `midas:sub:confirm:${telegramUserId}:${chatId}`;
            ...
            if (confirmData) {
              ...
              await redisConnection.del(`midas:sub:freq:${telegramUserId}:${chatId}`);
              if (remMsgId) void editMessageText(chatId, remMsgId, `✅ Подписка создана!\n\n🎬 ${name} · ${amount} ${currency}`, {
                inline_keyboard: [[{ text: '🔙 К подпискам', callback_data: 'sub:list' }]],
              });
            }
          }
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, `✅ Записано! ${detail?.itemName ?? '—'} · ${detail?.amount ?? ''} ${detail?.currency ?? ''}`, {
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId, '⏭ Пропущено.', {
              inline_keyboard: [[{ text: '✖️ Закрыть', callback_data: 'sub:close' }]],
            });
          }
            ...
            const { pauseRecurring } = await import('../services/recurring.service.js');
            await pauseRecurring(resolved.workspaceId, resolved.userId, recId);
            if (remMsgId) void editMessageText(chatId, remMsgId, '✖️ Подписка отменена.', {
              inline_keyboard: [[{ text: '✖️ Закрыть', callback_data: 'sub:close' }]],
            });
          }
          else if (sub === 'close') {
      ...
      // ── Phase 7.1: Financial Reminders callbacks (prefix "rem:") ─────
      // Handles: rem:list, rem:add, rem:type:{type}, rem:v:{id},
      //          rem:done:{id}, rem:snooze:{id}, rem:cancel:{id},
      //          rem:del:{id}, rem:delok:{id}
      if (callbackData.startsWith('rem:')) {
        try {
          const resolved = await resolveWorkspace(telegramUserId, chatId);
          const remSub = callbackData.slice(4); // everything after "rem:"
          const remMsgId = cq.message ? String(cq.message.message_id) : null;

          const {
            getReminderListScreen,
            getReminderDetail,
            buildReminderDetailScreen,
            completeReminder,
            snoozeReminder,
            cancelReminder,
            deleteReminder,
          } = await import('../services/reminder.service.js');

          // rem:list → show list screen
          if (remSub === 'list') {
            const screen = await getReminderListScreen(resolved.workspaceId, resolved.userId);
            if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
          }

          // rem:add → Step 1: ask for title (text intercept)
          else if (remSub === 'add') {
            const text = '📅 <b>Новое напоминание</b>\n\nВведите название:\n\n❯ Например: Аренда, Netflix, Долг Влада';
            const kb = { inline_keyboard: [[{ text: '🔙 Отмена', callback_data: 'rem:list' }]] };
            if (remMsgId) void editMessageText(chatId, remMsgId, text, kb);
            await redisConnection.set(
              `midas:rem:title:${telegramUserId}:${chatId}`,
              String(remMsgId),
              'EX', 120,
            );
          }

          // rem:type:{type} → Step 2 complete, go to Step 3: ask for amount
          else if (remSub.startsWith('type:')) {
            const remType = remSub.slice(5);
            if (!['expense', 'income', 'debt_pay', 'debt_receive'].includes(remType)) {
              void answerCallbackQuery(cq.id);
              await reply.status(200).send({ ok: true });
              return;
            }
            // Restore title from Redis key (set in text intercept step 1→2)
            const titleKey = `midas:rem:type:${telegramUserId}:${chatId}`;
            const titleRaw = await redisConnection.get(titleKey);
            if (!titleRaw) {
              if (remMsgId) void editMessageText(chatId, remMsgId, '⚠️ Сессия истекла. Начните заново.', { inline_keyboard: [[{ text: '📅 Напоминания', callback_data: 'rem:list' }]] });
            } else {
              await redisConnection.del(titleKey);
              const title = titleRaw;
              const text = `📅 <b>${title}</b>\n\nВведите сумму и валюту:\n\n❯ Например: 15000 UAH, 500 USD, 150`;
              const kb = { inline_keyboard: [[{ text: '🔙 Отмена', callback_data: 'rem:list' }]] };
              if (remMsgId) void editMessageText(chatId, remMsgId, text, kb);
              // Store: title:type
              await redisConnection.set(
                `midas:rem:amt:${telegramUserId}:${chatId}`,
                `${title}:::${remType}:::${String(remMsgId)}`,
                'EX', 120,
              );
            }
          }

          // rem:v:{id} → show detail screen
          else if (remSub.startsWith('v:')) {
            const remId = remSub.slice(2);
            ...
            if (detail) {
              const screen = buildReminderDetailScreen(detail);
              if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
            }
          }
          ...
          // rem:snooze:{id} → snooze by 1 day
          else if (remSub.startsWith('snooze:')) {
            const remId = remSub.slice(7);
            if (!/^[0-9A-Z]{26}$/.test(remId)) {
              void answerCallbackQuery(cq.id);
              await reply.status(200).send({ ok: true });
              return;
            }
            await snoozeReminder(resolved.workspaceId, resolved.userId, remId, 1);
            const updated = await getReminderDetail(resolved.workspaceId, resolved.userId, remId);
            if (updated) {
              const screen = buildReminderDetailScreen(updated);
              if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
            }
          }

          // rem:cancel:{id} → cancel reminder
          else if (remSub.startsWith('cancel:')) {
            const remId = remSub.slice(7);
            if (!/^[0-9A-Z]{26}$/.test(remId)) {
              void answerCallbackQuery(cq.id);
              await reply.status(200).send({ ok: true });
            ...
            await cancelReminder(resolved.workspaceId, resolved.userId, remId);
            const screen = await getReminderListScreen(resolved.workspaceId, resolved.userId);
            if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
          }

          // rem:del:{id} → confirm delete
          else if (remSub.startsWith('del:') && !remSub.startsWith('delok:')) {
            const remId = remSub.slice(4);
            if (!/^[0-9A-Z]{26}$/.test(remId)) {
              void answerCallbackQuery(cq.id);
              await reply.status(200).send({ ok: true });
              return;
            ...
            if (remMsgId) void editMessageText(chatId, remMsgId,
              ...
              { inline_keyboard: [
                [{ text: '✅ Да, удалить', callback_data: `rem:delok:${remId}` }],
                [{ text: '🔙 Отмена', callback_data: `rem:v:${remId}` }],
              ] },
            );
          }

          // rem:delok:{id} → confirmed delete
          else if (remSub.startsWith('delok:')) {
            const remId = remSub.slice(6);
            if (!/^[0-9A-Z]{26}$/.test(remId)) {
              void answerCallbackQuery(cq.id);
              await reply.status(200).send({ ok: true });
              return;
            }
            await deleteReminder(resolved.workspaceId, resolved.userId, remId);
            const screen = await getReminderListScreen(resolved.workspaceId, resolved.userId);
            if (remMsgId) void editMessageText(chatId, remMsgId, screen.text, screen.keyboard);
          }
          ...
          const errorClass = err instanceof Error ? err.constructor.name : 'UnknownError';
          request.log.error({ msg: '[midas:bot:webhook] rem: callback failed', callbackId: cq.id, errorClass });
        }
      ...
      if (navCmd) {
        try {
          ...
          if (navCmd === 'transactions') {
            ...
            navCmd === 'edit_last' ||
            navCmd === 'edit_amount' ||
            navCmd === 'edit_category' ||
            navCmd === 'edit_account' ||
            navCmd === 'edit_type'
          ) {
            // ── Phase 5.0-B: Quick Edit + edit_last combined handler ──────────
            // buildCommandResponse returns either a full card (edit_last)
            // or a sentinel string __QUICK_EDIT__:{field}:{txId} (Phase 5.0).
            const elResponse = await buildCommandResponse(navCmd, cmdCtx);

            if (elResponse.text.startsWith('__QUICK_EDIT__:')) {
              // ── Send-First Bridge: open the right picker as a NEW message ──
              const qeParts = elResponse.text.split(':');
              const qeField = qeParts[1] ?? '';   // 'amt' | 'cat' | 'acc' | 'int'
              const qeTxId  = qeParts[2] ?? '';   // 26-char ULID

              await handleQuickEditField(
                chatId, telegramUserId, qeTxId, qeField,
                resolved.workspaceId, resolved.userId,
              );

              request.log.info({
                msg: '[midas:bot:webhook] nav:quick_edit',
                telegramUserId,
                field: qeField,
                txId: qeTxId,
                workspaceId: resolved.workspaceId,
              ...
              // ── edit_last: full card with edit buttons ──
    ...
    if (!commandToken) {
      ...
      if (acRaw) {
        ...
        // ── rename_for_dup: user is renaming after "already exists" ──────
        // Currency is already known — skip fuzzy match + currency picker.
        if (acState.step === 'rename_for_dup' && acState.currency) {
          const raw = message.text.trim();
          const trimmed = capitalizeFirst(raw);
          if (raw.length === 0) {
            void upsertBotMessage(telegramUserId, chatId,
              '🤔 Введите новое название для счёта:');
            await reply.status(200).send({ ok: true });
            return;
          }
          if (trimmed.length > 100) {
            void upsertBotMessage(telegramUserId, chatId,
              '⚠️ Название слишком длинное (макс. 100 символов). Попробуй покороче:');
            await reply.status(200).send({ ok: true });
            return;
          }

          let resolved: { workspaceId: string; userId: string };
          try {
            resolved = await resolveWorkspace(telegramUserId, chatId);
          } catch {
            await reply.status(200).send({ ok: true });
            return;
          }

          const renRes = await addAccountReturningId(
            resolved.workspaceId, resolved.userId, trimmed, acState.currency,
          );

          if (renRes.status === 'already_exists') {
            // Still duplicate — show error again, stay in rename_for_dup
            const dupState2: AccountOnboardState = { ...acState, name: trimmed };
            await redisConnection.set(acKey, JSON.stringify(dupState2), 'EX', ONBOARD_STATE_TTL_SEC);
            void upsertBotMessage(telegramUserId, chatId,
              `⚠️ Счёт <b>${escapeHtml(trimmed)} · ${escapeHtml(acState.currency)}</b> тоже уже существует.\n\n` +
              `<i>Введите другое название:</i>`);
            await reply.status(200).send({ ok: true });
            return;
          }

          // Account created (or child created) — proceed to bal_input
          try {
            await softDeletePlaceholderAccount(resolved.workspaceId, resolved.userId);
          } catch { /* non-fatal */ }

          const effName = renRes.finalName ?? trimmed;
          const balState: AccountOnboardState = {
            step: 'bal_input',
            accountType: acState.accountType,
            name: effName,
            accountId: renRes.accountId,
            currency: acState.currency,
            linkedDraftId: acState.linkedDraftId,
          };
          await redisConnection.set(acKey, JSON.stringify(balState), 'EX', ONBOARD_STATE_TTL_SEC);

          const prefix = renRes.status === 'created_as_child'
            ? `✅ Добавлена валюта <b>${escapeHtml(acState.currency)}</b> к счёту <b>${escapeHtml(trimmed)}</b>\n\nСчёт: <b>${escapeHtml(effName)}</b>\n\n`
            : `✅ Счёт <b>${escapeHtml(effName)} · ${escapeHtml(acState.currency)}</b> создан.\n\n`;
          void upsertBotMessage(telegramUserId, chatId,
            prefix + '💰 Введите начальный баланс\nили нажмите «Пропустить»:',
    ...
    // ── Phase 7.1: Reminder FSM text intercepts ────────────────────────────────
    // Step 1: Title → show type picker
    if (!commandToken) {
      const remTitleKey = `midas:rem:title:${telegramUserId}:${chatId}`;
      const remTitleMsgId = await redisConnection.get(remTitleKey);
      if (remTitleMsgId) {
        await redisConnection.del(remTitleKey);
        const title = message.text.trim().slice(0, 100);
        if (!title) {
          void upsertBotMessage(telegramUserId, chatId, '⚠️ Название не может быть пустым. Попробуйте ещё раз.');
          await redisConnection.set(remTitleKey, remTitleMsgId, 'EX', 120);
        } else {
          // Store title in next step key, then show type picker
          await redisConnection.set(
            `midas:rem:type:${telegramUserId}:${chatId}`,
            title,
            'EX', 120,
          );
          const text = `📅 <b>${title}</b>\n\nВыберите тип:`;
          const kb = {
            inline_keyboard: [
              [
                { text: '💸 Расход', callback_data: 'rem:type:expense' },
                { text: '💰 Доход',  callback_data: 'rem:type:income' },
              ],
              [
                { text: '🤝 Отдать долг', callback_data: 'rem:type:debt_pay' },
                { text: '🤲 Получить долг', callback_data: 'rem:type:debt_receive' },
              ],
              [{ text: '🔙 Отмена', callback_data: 'rem:list' }],
            ],
          };
          void upsertBotMessage(telegramUserId, chatId, text, kb);
        }
        await reply.status(200).send({ ok: true });
        return;
      }
    ...
    // Step 2 (amount): title:::type:::msgId stored in midas:rem:amt
    if (!commandToken) {
      const remAmtKey = `midas:rem:amt:${telegramUserId}:${chatId}`;
      const remAmtState = await redisConnection.get(remAmtKey);
      if (remAmtState) {
        const [title, remType] = remAmtState.split(':::') as [string, string, string];
        if (!title || !remType) {
          await redisConnection.del(remAmtKey);
        } else {
          const { parseAmountCurrency } = await import('../services/reminder.service.js');
          const parsed = parseAmountCurrency(message.text.trim());
          if (!parsed) {
            void upsertBotMessage(telegramUserId, chatId,
              '⚠️ Не удалось распознать сумму. Попробуйте: <b>15000 UAH</b> или <b>500 USD</b>',
              { inline_keyboard: [[{ text: '🔙 Отмена', callback_data: 'rem:list' }]] },
            );
            // Keep key alive
            await redisConnection.expire(remAmtKey, 120);
          } else {
            await redisConnection.del(remAmtKey);
            // Store: title:::type:::amount:::currency for Step 3
            await redisConnection.set(
              `midas:rem:date:${telegramUserId}:${chatId}`,
              `${title}:::${remType}:::${String(parsed.amount)}:::${parsed.currency}`,
              'EX', 120,
            );
            void upsertBotMessage(telegramUserId, chatId,
              `📅 <b>${title}</b> · ${String(parsed.amount)} ${parsed.currency}\n\nВведите дату:\n\n❯ <code>15.06</code>  <code>через 3 дня</code>  <code>завтра</code>  <code>5 числа</code>`,
              { inline_keyboard: [[{ text: '🔙 Отмена', callback_data: 'rem:list' }]] },
            );
          }
          await reply.status(200).send({ ok: true });
    ...
    // Step 3 (date): title:::type:::amount:::currency stored in midas:rem:date
    if (!commandToken) {
      const remDateKey = `midas:rem:date:${telegramUserId}:${chatId}`;
      const remDateState = await redisConnection.get(remDateKey);
      if (remDateState) {
        const parts = remDateState.split(':::') as [string, string, string, string];
        const [title, remType, amountStr, currency] = parts;
        if (!title || !remType || !amountStr || !currency) {
          await redisConnection.del(remDateKey);
        } else {
          const { parseRussianDate, createReminder } = await import('../services/reminder.service.js');
          const parsedDate = parseRussianDate(message.text.trim());
          if (!parsedDate) {
            void upsertBotMessage(telegramUserId, chatId,
              '⚠️ Не удалось распознать дату. Попробуйте: <b>15.06</b>, <b>через 3 дня</b>, <b>завтра</b>, <b>5 числа</b>',
              { inline_keyboard: [[{ text: '🔙 Отмена', callback_data: 'rem:list' }]] },
            );
            await redisConnection.expire(remDateKey, 120);
          } else {
            await redisConnection.del(remDateKey);
            try {
              const resolved = await resolveWorkspace(telegramUserId, chatId);
              const remId = await createReminder(resolved.workspaceId, resolved.userId, {
                title,
                amount: parseFloat(amountStr),
                currency,
                reminderType: remType as 'expense' | 'income' | 'debt_pay' | 'debt_receive',
                dueDate: parsedDate,
              });
              const { getReminderDetail, buildReminderDetailScreen } = await import('../services/reminder.service.js');
              const detail = await getReminderDetail(resolved.workspaceId, resolved.userId, remId);
              if (detail) {
                const screen = buildReminderDetailScreen(detail);
                const successText = `✅ <b>Напоминание создано!</b>\n\n${screen.text}`;
                void upsertBotMessage(telegramUserId, chatId, successText, screen.keyboard);
              } else {
                void upsertBotMessage(telegramUserId, chatId,
                  '✅ Напоминание создано!',
                  { inline_keyboard: [[{ text: '📅 Мои напоминания', callback_data: 'rem:list' }]] },
                );
              }
            } catch (err: unknown) {
              const errorClass = err instanceof Error ? err.constructor.name : 'UnknownError';
              request.log.error({ msg: '[midas:bot:webhook] rem:date createReminder failed', errorClass });
              void upsertBotMessage(telegramUserId, chatId, '⚠️ Не удалось создать напоминание. Попробуйте позже.');
            }
          }
          await reply.status(200).send({ ok: true });
          return;
...
//
// SEC-12: No user-content is logged. Only chatId, telegramUserId, field, txId.
// ─────────────────────────────────────────────────────────────────────────────

async function handleQuickEditField(
  chatId: string,
  telegramUserId: string,
  txId: string,
  field: string,  // 'amt' | 'cat' | 'acc' | 'int'
  workspaceId: string,
  userId: string,
): Promise<void> {
  ...
  if (field === 'amt') {
    ...
    } catch { /* non-fatal: text intercept falls back to upsertBotMessage */ }
  ...
  if (field === 'cat') {
    const [allCats, card] = await Promise.all([
      ...
      getTransactionCard(txId, workspaceId, userId),
    ]);
